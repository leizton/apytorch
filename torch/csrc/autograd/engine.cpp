...
auto Engine::execute(
  ...
  // Queue the root
  execute_with_graph_task(graph_task, std::move(graph_root), InputBuffer(variable_list()));
...
auto Engine::ready_queue(
  ...
  bool multithreading_disabled = !c10::AutogradState::get_tls_state().get_multithreading_enabled();
