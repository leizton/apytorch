...
variable_list AbsBackward0::apply_with_saved(
...
variable_list AcosBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list AddBackward0::apply(variable_list&& grad_outputs) {
  // grad_outputs: 输出张量关于loss的梯度
  // grad_inputs:  输入张量关于loss的梯度
  variable_list grad_inputs(2);
  grad_inputs[1] = handle_r_to_c(other_scalar_type, maybe_multiply(grad_outputs[0], alpha.conj()));
  grad_inputs[0] = handle_r_to_c(self_scalar_type, grad_outputs[0]);
  return grad_inputs;
  /*
  IndexRangeGenerator gen;
  ...
  if (task_should_compute_output({other_ix})) {
    auto grad_result = any_grad_defined
        ? (handle_r_to_c(other_scalar_type, maybe_multiply(grad, alpha.conj())))
        : Tensor();
...
variable_list AddBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
...
variable_list AddBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (handle_r_to_c(self_scalar_type, grad)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list AddBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_scalar_type);
  variable_list result = apply(variable_list(grads));
...
variable_list AddbmmBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({batch1_ix})) {
    auto grad_result = any_grad_defined
        ? (maybe_multiply(
              grad.unsqueeze(0)
                  .expand_symint(
                      {batch1_sym_argsize_0,
                       batch1_sym_argsize_1,
                       batch2_sym_argsize_2})
                  .bmm(batch2.transpose(1, 2).conj()),
              alpha.conj()))
        : Tensor();
  ...
  if (task_should_compute_output({batch2_ix})) {
    auto grad_result = any_grad_defined
        ? (maybe_multiply(
              batch1.transpose(1, 2).conj().bmm(grad.unsqueeze(0).expand_symint(
                  {batch1_sym_argsize_0,
                   batch1_sym_argsize_1,
                   batch2_sym_argsize_2})),
              alpha.conj()))
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (maybe_multiply(grad, beta.conj())) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list AddbmmBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
...
variable_list AddcdivBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({tensor2_ix})) {
    auto grad_result = any_grad_defined
        ? (handle_r_to_c(
              tensor2_scalar_type,
              -grad * (value * tensor1 / (tensor2 * tensor2)).conj()))
        : Tensor();
...
variable_list AddcdivBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_scalar_type);
...
variable_list AddcmulBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_scalar_type);
  saved.before(tensor1_);
...
variable_list AddmmBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({mat1_ix})) {
    auto grad_result = any_grad_defined
        ? (mm_mat1_backward(
              grad, mat2, mat1_sym_sizes, mat1_sym_strides, mat1_layout, alpha))
        : Tensor();
  ...
  if (task_should_compute_output({mat2_ix})) {
    auto grad_result = any_grad_defined
        ? (mm_mat2_backward(
              grad, mat1, mat2_sym_sizes, mat2_sym_strides, mat2_layout, alpha))
        : Tensor();
...
variable_list AddmmBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
  saved.before(beta);
...
variable_list SparseAddmmBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({mat1_ix})) {
    auto grad_result = any_grad_defined
        ? (mm_mat1_sparse_backward(grad, mat1, mat2, alpha))
        : Tensor();
    copy_range(grad_inputs, mat1_ix, grad_result);
  }
  if (task_should_compute_output({mat2_ix})) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (maybe_multiply(grad, beta)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SparseAddmmBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
  saved.before(beta);
...
variable_list AddmvBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({mat_ix})) {
    auto grad_result = any_grad_defined
        ? (maybe_multiply(grad.ger(vec.conj()), alpha.conj()))
        : Tensor();
  ...
  if (task_should_compute_output({vec_ix})) {
    auto grad_result = any_grad_defined
        ? (maybe_multiply(mat.t().conj().mv(grad), alpha.conj()))
        : Tensor();
...
variable_list AddmvBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
  saved.before(beta);
...
variable_list AddrBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
  saved.before(beta);
...
variable_list AffineGridGeneratorBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({theta_ix})) {
    auto grad_result = any_grad_defined
        ? (affine_grid_generator_backward_symint(grad, size, align_corners))
        : Tensor();
...
variable_list AffineGridGeneratorBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list AliasBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list AngleBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (angle_backward(grad, self)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list AngleBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list AcoshBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (self.is_complex()
               ? grad * ((self + 1).rsqrt() * (self - 1).rsqrt()).conj()
               : grad * (self * self - 1).rsqrt())
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list AcoshBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list AcoshBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list AsinhBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad * (self.pow(2) + 1).rsqrt().conj()) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list AsinhBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list AsinhBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list AtanhBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad * 1 / (1 - self.pow(2)).conj()) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list AtanhBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list AtanhBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list AsStridedBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (as_strided_backward(
              grad, self_geometry, size, stride, storage_offset))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list AsStridedBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list AsStridedBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_geometry);
  saved.before(size);
...
variable_list AsinBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list AtanBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad / (self * self + 1).conj()) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list AtanBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list Atan2Backward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list BaddbmmBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({batch1_ix})) {
    auto grad_result = any_grad_defined
        ? (maybe_multiply(
              grad.bmm(batch2.transpose(1, 2).conj()), alpha.conj()))
        : Tensor();
    copy_range(grad_inputs, batch1_ix, grad_result);
  }
  if (task_should_compute_output({batch2_ix})) {
    auto grad_result = any_grad_defined
        ? (maybe_multiply(
              batch1.transpose(1, 2).conj().bmm(grad), alpha.conj()))
        : Tensor();
    copy_range(grad_inputs, batch2_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
...
variable_list BaddbmmBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
  saved.before(batch1_);
...
variable_list BernoulliBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list BernoulliBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(p_info);
  variable_list result = apply(variable_list(grads));
...
variable_list BernoulliBackward2::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list BmmBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({mat2_ix})) {
    auto grad_result =
        any_grad_defined ? (self.transpose(1, 2).conj().bmm(grad)) : Tensor();
    copy_range(grad_inputs, mat2_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad.bmm(mat2.transpose(1, 2).conj())) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list BmmBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(mat2_);
  saved.before(self_);
...
variable_list MatmulBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list CatBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
...
variable_list CauchyBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list CeilBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list CholeskyBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (cholesky_backward(grad, upper, result)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list CholeskyBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(upper);
...
variable_list LinalgCholeskyExBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (cholesky_backward(grad, upper, L)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list LinalgCholeskyExBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(upper);
  saved.before(L_);
...
variable_list CholeskySolveBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix, input2_ix})) {
    ...
    auto grad_result = cholesky_solve_backward(
        grad, self, input2, result, upper, grad_input_mask);
    if (task_should_compute_output({self_ix})) {
      copy_range(grad_inputs, self_ix, std::get<0>(grad_result));
    }
...
variable_list CholeskySolveBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list CholeskyInverseBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (cholesky_inverse_backward(grad, self, upper, result))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list CholeskyInverseBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(upper);
...
variable_list ClampBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({min_ix, max_ix})) {
    ...
    auto grad_result =
        clamp_backward_min_max(grad, self, min, max, grad_input_mask);
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (clamp_backward(grad, self, min, max)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list ClampBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(max_);
  saved.before(min_);
...
variable_list ClampBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(max);
  saved.before(min);
...
variable_list ClampMinBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (where(self >= min, grad, at::scalar_tensor(0., grad.options())))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list ClampMinBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(min);
  saved.before(self_);
...
variable_list ClampMinBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(min_);
  saved.before(self_);
...
variable_list ClampMaxBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (where(self <= max, grad, at::scalar_tensor(0., grad.options())))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list ClampMaxBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(max);
  saved.before(self_);
...
variable_list ClampMaxBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(max_);
  saved.before(self_);
...
variable_list CloneBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list ToCopyBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (_to_copy_backward(grad, self_options)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list ToCopyBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list CoalesceBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list ComplexBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(imag_);
  saved.before(real_);
...
variable_list PolarBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list ConjBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list NegViewBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list ConjPhysicalBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list ConjPhysicalBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list CopysignBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (copysign_tensor_self_backward(grad, self, result))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list CopysignBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list CopysignBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(result_);
...
variable_list CosBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad * -self.sin().conj()) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list CosBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list CoshBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad * self.sinh().conj()) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list CoshBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list LinalgCrossBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    auto grad_result = any_grad_defined
        ? (at::linalg_cross(grad, self.conj(), dim))
        : Tensor();
    copy_range(grad_inputs, other_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (at::linalg_cross(other.conj(), grad, dim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list LinalgCrossBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(other_);
...
variable_list LogcumsumexpBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (logcumsumexp_backward(grad, self, result, dim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list LogcumsumexpBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_);
...
variable_list CumprodBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (cumprod_backward(grad.to(self_scalar_type), self, dim, result))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list CumprodBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_);
...
variable_list CumsumBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (cumsum_backward(grad.to(self_scalar_type), dim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list CumsumBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_scalar_type);
...
variable_list CummaxBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (cummaxmin_backward(grad, self, indices, dim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list CummaxBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_);
...
variable_list CumminBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_);
...
variable_list ConvTbcBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix, weight_ix, bias_ix})) {
    auto grad_result = grad.defined()
        ? conv_tbc_backward(grad, self, weight, bias, pad)
...
variable_list ConvTbcBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(bias_);
  saved.before(pad);
...
variable_list CtcLossBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({log_probs_ix})) {
    auto grad_result = any_grad_defined ? (_ctc_loss_backward(
                                              grad,
                                              log_probs,
                                              targets,
                                              input_lengths,
...
variable_list CtcLossBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(blank);
...
variable_list CtcLossBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(blank);
...
variable_list Deg2RadBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list LinalgDetBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({A_ix})) {
    auto grad_result = any_grad_defined
        ? (linalg_det_backward(grad, result, A, LU, pivots))
        : Tensor();
...
variable_list LinalgDetBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(A_);
  saved.before(LU_);
...
variable_list LinalgSlogdetBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({A_ix})) {
    auto grad_result = any_grad_defined
        ? (slogdet_backward(grad_sign, grad_logabsdet, A, sign, LU, pivots))
        : Tensor();
    copy_range(grad_inputs, A_ix, grad_result);
  }
  return grad_inputs;
...
variable_list LinalgSlogdetBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(A_);
  saved.before(LU_);
...
variable_list BlockDiagBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({tensors_ix})) {
    auto grad_result =
        block_diag_backward(grad, tensors_args_sizes, tensors_args_scalartypes);
    copy_range(grad_inputs, tensors_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list BlockDiagBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(tensors_args_scalartypes);
...
variable_list DiagEmbedBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad.diagonal(offset, dim1, dim2)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list DiagEmbedBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim1);
  saved.before(dim2);
...
variable_list DiagonalBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (diagonal_backward_symint(grad, self_sym_sizes, offset, dim1, dim2))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list DiagonalBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim1);
  saved.before(dim2);
...
variable_list DiagonalBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim1);
  saved.before(dim2);
...
variable_list DistBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    auto grad_result = any_grad_defined
        ? (-norm_backward(grad, self - other, p, result))
        : Tensor();
    copy_range(grad_inputs, other_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
...
variable_list DistBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(p);
...
variable_list DivBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list DivBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other);
  saved.before(self_scalar_type);
  ...
  if (task_should_compute_output({other_ix})) {
    auto grad_result = any_grad_defined
        ? (div_tensor_other_backward(
              ...
              self,
              other,
              rounding_mode.has_value()
                  ? c10::optional<c10::string_view>(rounding_mode.value())
                  : c10::nullopt))
        : Tensor();
    copy_range(grad_inputs, other_ix, grad_result);
  }
...
variable_list DivBackward2::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
...
variable_list DivBackward3::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other);
  saved.before(rounding_mode);
...
variable_list DotBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
...
variable_list VdotBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list FusedDropoutBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (_fused_dropout_backward(grad, result1, p))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list FusedDropoutBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(p);
  saved.before(result1_);
...
variable_list NativeDropoutBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({input_ix})) {
    auto grad_result = any_grad_defined
        ? (GradMode::is_enabled()
               ? infinitely_differentiable_native_dropout_backward(
                     grad,
                     result1,
                     (!train.has_value() || !train.value()
                          ? 1
                          : (p == 1 ? 0.0 : 1.0 / (1.0 - p))))
...
variable_list NativeDropoutBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(p);
  saved.before(train);
...
variable_list NativeDropoutBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (native_dropout_double_backward(grad, grad_output, mask, scale))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
...
variable_list NativeDropoutBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list EqBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list EqBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_info);
  saved.before(self_info);
...
variable_list ErfBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (2.0 / sqrt(M_PI) * exp(-(self.pow(2))) * grad)
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list ErfBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ErfcBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list SpecialErfcxBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? ((2.0 * self * result - 2.0 / sqrt(M_PI)) * grad)
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SpecialErfcxBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(result_);
...
variable_list ErfinvBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (0.5 * sqrt(M_PI) * exp(self.erfinv().pow(2)) * grad)
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list ErfinvBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ExpBackward0::apply(variable_list&& grad_outputs) {
  // grad_outputs: 输出张量关于loss的梯度
  // grad_inputs:  输入张量关于loss的梯度
  std::lock_guard<std::mutex> lock(mutex_);
  auto result = result_.unpack(shared_from_this());
  auto grad_input = grad_outputs[0] * result.conj();
  return variable_list{grad_input};
  /*
  std::lock_guard<std::mutex> lock(mutex_);
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  ...
  const auto& grad = grads[0];
  auto result = result_.unpack(shared_from_this());
  // 是否有任意grad设置了requires_grad
  bool any_grad_defined = any_variable_defined(grads);
  // task_should_compute_output: 不是终节点
  if (task_should_compute_output({self_ix})) {
...
variable_list ExpBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list Exp2Backward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list Expm1Backward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad * (result.conj() + 1)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list Expm1Backward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list ExpandBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (at::sum_to(grad, self_sym_sizes)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list ExpandBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_sym_sizes);
  variable_list result = apply(variable_list(grads));
...
variable_list ExponentialBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list FakeQuantizePerTensorAffineCachemaskBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (fake_quantize_per_tensor_affine_cachemask_backward(grad, mask))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
void FakeQuantizePerTensorAffineCachemaskBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(mask_);
}
variable_list FakeQuantizePerTensorAffineCachemaskBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(mask_);
  variable_list result = apply(variable_list(grads));
...
variable_list FakeQuantizePerTensorAffineCachemaskTensorQparamsBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void FakeQuantizePerTensorAffineCachemaskTensorQparamsBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(mask_);
}
variable_list FakeQuantizePerTensorAffineCachemaskTensorQparamsBackward0::
    apply_with_saved(const variable_list& grads, SwapSavedVariables& saved) {
  saved.before(mask_);
  variable_list result = apply(variable_list(grads));
  saved.after(mask_);
  return result;
}
variable_list FakeQuantizeLearnablePerTensorAffineBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (task_should_compute_output({self_ix, scale_ix, zero_point_ix})) {
    auto grad_result = grad.defined()
        ? _fake_quantize_learnable_per_tensor_affine_backward(
              grad, self, scale, zero_point, quant_min, quant_max, grad_factor)
        : std::tuple<Tensor, Tensor, Tensor>();
    if (task_should_compute_output({self_ix})) {
      copy_range(grad_inputs, self_ix, std::get<0>(grad_result));
    }
...
void FakeQuantizeLearnablePerTensorAffineBackward0::compiled_args(
    CompiledNodeArgs& args) {
...
variable_list FakeQuantizeLearnablePerTensorAffineBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list FakeQuantizePerChannelAffineCachemaskBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (fake_quantize_per_channel_affine_cachemask_backward(grad, mask))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
void FakeQuantizePerChannelAffineCachemaskBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(mask_);
}
variable_list FakeQuantizePerChannelAffineCachemaskBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(mask_);
  variable_list result = apply(variable_list(grads));
...
variable_list FakeQuantizeLearnablePerChannelAffineBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void FakeQuantizeLearnablePerChannelAffineBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(axis);
  args.collect(grad_factor);
  args.collect(quant_max);
...
variable_list FakeQuantizeLearnablePerChannelAffineBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(axis);
  saved.before(grad_factor);
...
variable_list FusedMovingAvgObsFqHelperBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(mask_);
  variable_list result = apply(variable_list(grads));
...
variable_list FillBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list FillBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list FillBackward2::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list FillBackward3::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list FloorBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list FmodBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list FmodBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    auto grad_result = any_grad_defined
        ? (-grad * self.div(other, /*rounding_mode=*/"trunc"))
        : Tensor();
    copy_range(grad_inputs, other_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
...
variable_list FmodBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list FracBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list FrexpBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list GatherBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (gather_backward(grad, self, dim, index, sparse_grad))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list GatherBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(index_);
...
variable_list GeBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_info);
  variable_list result = apply(variable_list(grads));
...
variable_list GeBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_info);
  saved.before(self_info);
...
variable_list GeometricBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list GeqrfBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list GridSampler2DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({input_ix, grid_ix})) {
    ...
    auto grad_result = grad.defined() ? grid_sampler_2d_backward(
                                            grad,
                                            input,
                                            grid,
                                            interpolation_mode,
...
variable_list GridSampler2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(align_corners);
  saved.before(grid_);
...
variable_list GridSampler3DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({input_ix, grid_ix})) {
    ...
    auto grad_result = grad.defined() ? grid_sampler_3d_backward(
                                            grad,
                                            input,
                                            grid,
                                            interpolation_mode,
...
variable_list GridSampler3DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(align_corners);
  saved.before(grid_);
...
variable_list GridSampler2DCpuFallbackBackward0::apply(variable_list&& grads) {
  ...
  auto input = input_.unpack();
  if (task_should_compute_output({input_ix, grid_ix})) {
    auto grad_result = grad.defined() ? _grid_sampler_2d_cpu_fallback_backward(
                                            grad,
                                            input,
                                            grid,
                                            interpolation_mode,
...
variable_list GridSampler2DCpuFallbackBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(align_corners);
  saved.before(grid_);
...
variable_list GtBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_info);
  variable_list result = apply(variable_list(grads));
...
variable_list GtBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_info);
  saved.before(self_info);
...
variable_list HardsigmoidBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (hardsigmoid_backward(grad, self)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list HardsigmoidBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list HardswishBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (hardswish_backward(grad, self)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list HardswishBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list HardswishBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (at::where(
              at::logical_and(-3.0 < self, self < 3.0),
              grad * grad_output / 3.0,
              at::zeros({}, self_options)))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list HardswishBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(grad_output_);
  saved.before(self_);
...
variable_list HypotBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list I0Backward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad * at::special_i1(self)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list I0Backward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list SpecialI0EBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (grad * (at::special_i1e(self) - self.sgn() * result))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SpecialI0EBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(result_);
...
variable_list SpecialI1Backward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (i1_backward(grad, self, result)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SpecialI1Backward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(result_);
...
variable_list SpecialI1EBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (i1e_backward(grad, self, result)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SpecialI1EBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(result_);
...
variable_list IgammaBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list IgammacBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list IndexBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (index_backward(
              grad.new_zeros_symint(self_sym_sizes, self_options),
              indices,
              grad))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
...
variable_list IndexBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(indices_);
  saved.before(self_options);
...
variable_list UnsafeIndexBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(indices_);
  saved.before(self_options);
...
variable_list IndexAddBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({source_ix})) {
    auto grad_result = any_grad_defined
        ? (maybe_multiply(
              source_dim > 0 ? grad.index_select(dim, index).expand_as(source)
                             : grad.index_select(dim, index.squeeze(0)),
              alpha))
        : Tensor();
...
variable_list IndexAddBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
  saved.before(dim);
...
variable_list IndexReduceBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix, source_ix})) {
    auto grad_result = index_reduce_backward(
        grad, self, dim, index, source, reduce, include_self, result);
    if (task_should_compute_output({self_ix})) {
      copy_range(grad_inputs, self_ix, std::get<0>(grad_result));
    }
    if (task_should_compute_output({source_ix})) {
...
variable_list IndexReduceBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
...
variable_list IndexCopyBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad.index_fill(dim, index, 0)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  if (task_should_compute_output({source_ix})) {
    auto grad_result = any_grad_defined
...
variable_list IndexCopyBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(index_);
...
variable_list IndexFillBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(index_);
...
variable_list IndexFillBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({value_ix})) {
    auto grad_result = any_grad_defined
        ? (grad.index_select(
                   dim, std::get<0>(at::_unique(index, /*sorted=*/false)))
               .sum())
        : Tensor();
    copy_range(grad_inputs, value_ix, grad_result);
  }
...
variable_list IndexFillBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(index_);
...
variable_list IndexPutBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (accumulate ? grad
                      : grad.index_put(indices, values_info.zeros(), false))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
...
variable_list IndexPutBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list UnsafeIndexPutBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (accumulate ? grad
                      : at::_unsafe_index_put(
                            grad, indices, values_info.zeros(), false))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  if (task_should_compute_output({values_ix})) {
    auto grad_result =
        any_grad_defined ? (at::_unsafe_index(grad, indices)) : Tensor();
    copy_range(grad_inputs, values_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list UnsafeIndexPutBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(accumulate);
  saved.before(indices_);
...
variable_list IndexPutImplBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(accumulate);
  saved.before(indices_);
...
variable_list IndexSelectBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (index_select_backward_symint(grad, self_sym_sizes, dim, index))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list IndexSelectBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(index_);
...
variable_list LinalgInvExBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({A_ix})) {
    auto grad_result = any_grad_defined
        ? (-at::matmul(inverse.mH(), at::matmul(grad, inverse.mH())))
        : Tensor();
    copy_range(grad_inputs, A_ix, grad_result);
  }
  return grad_inputs;
...
variable_list LinalgInvExBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list LinalgPinvBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (pinv_backward(grad, result, self)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list LinalgPinvBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(result_);
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (value_selecting_reduction_backward_symint(
              grad, dim, indices, self_sym_sizes, keepdim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list KthvalueBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
...
variable_list LeBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_info);
  variable_list result = apply(variable_list(grads));
...
variable_list LeBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_info);
  saved.before(self_info);
...
variable_list LerpBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (weight.isComplex() ? grad * (1 - weight.conj().toComplexDouble())
                              : grad * (1 - weight.toDouble()))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list LerpBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(weight);
  variable_list result = apply(variable_list(grads));
...
variable_list LerpBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad * (1 - weight).conj()) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  if (task_should_compute_output({weight_ix})) {
    auto grad_result =
        any_grad_defined ? (grad * (end - self).conj()) : Tensor();
...
variable_list LerpBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(end_);
  saved.before(self_);
...
variable_list LgammaBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list DigammaBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad * polygamma(1, self)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list DigammaBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list PolygammaBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad * polygamma(n + 1, self)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list PolygammaBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(n);
  saved.before(self_);
...
variable_list PolygammaBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(n);
  saved.before(self_);
...
variable_list LogBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list Log10Backward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (grad / (self.conj() * 2.3025850929940456))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list Log10Backward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list Log1PBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (log1p_backward(grad, self)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list Log1PBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list Log2Backward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (grad / (self.conj() * 0.6931471805599453))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list Log2Backward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list LogaddexpBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    auto grad_result =
        any_grad_defined ? (grad / (1 + exp(self - other)).conj()) : Tensor();
    copy_range(grad_inputs, other_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad / (1 + exp(other - self)).conj()) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list LogaddexpBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list Logaddexp2Backward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    auto grad_result =
        any_grad_defined ? (grad / (1 + pow(2, self - other))) : Tensor();
    copy_range(grad_inputs, other_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad / (1 + pow(2, other - self))) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list Logaddexp2Backward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list XlogyBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (at::xlogy(grad, other).masked_fill((self == 0.) & (other <= 0.), 0.))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list XlogyBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list XlogyBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self);
...
variable_list XlogyBackward2::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other);
  saved.before(self_);
...
variable_list SpecialXlog1PyBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    auto grad_result =
        any_grad_defined ? (grad * self / (other + 1)) : Tensor();
    copy_range(grad_inputs, other_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (at::special_xlog1py(grad, other)
               .masked_fill((self == 0.) & (other <= -1.), 0.))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SpecialXlog1PyBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list SpecialXlog1PyBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self);
...
variable_list SpecialXlog1PyBackward2::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other);
  saved.before(self_);
...
variable_list SpecialZetaBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    auto grad_result = any_grad_defined
        ? (grad * -self * special_zeta(self + 1., other))
        : Tensor();
    copy_range(grad_inputs, other_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
...
variable_list SpecialZetaBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list SpecialZetaBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    auto grad_result = any_grad_defined
        ? (grad * -self * special_zeta(self.toDouble() + 1., other))
        : Tensor();
    copy_range(grad_inputs, other_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SpecialZetaBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self);
...
variable_list SpecialZetaBackward2::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list LogNormalBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list LogsumexpBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (logsumexp_backward(grad, self, result, dim, keepdim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list LogsumexpBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(keepdim);
...
variable_list LinalgLstsqBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(b_);
  saved.before(self_);
...
variable_list LtBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_info);
  variable_list result = apply(variable_list(grads));
...
variable_list LtBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_info);
  saved.before(self_info);
...
variable_list LinalgLuFactorExBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({A_ix})) {
    auto grad_result = any_grad_defined
        ? (lu_factor_ex_backward(grad, LU, pivots, pivot))
        : Tensor();
    copy_range(grad_inputs, A_ix, grad_result);
  }
  return grad_inputs;
...
variable_list LinalgLuFactorExBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(pivot);
  saved.before(LU_);
...
variable_list LinalgLuBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({A_ix})) {
    auto grad_result = any_grad_defined
        ? (linalg_lu_backward(grad_L, grad_U, P, L, U, pivot))
        : Tensor();
    copy_range(grad_inputs, A_ix, grad_result);
  }
  return grad_inputs;
...
variable_list LinalgLuBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(pivot);
  saved.before(L_);
...
variable_list LinalgLuSolveBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({LU_ix})) {
    auto grad_result = any_grad_defined
        ? (linalg_lu_solve_LU(grad, LU, pivots, result, left, adjoint))
        : Tensor();
...
variable_list LinalgLuSolveBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(LU_);
...
variable_list LuUnpackBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({LU_data_ix})) {
    auto grad_result = any_grad_defined ? (lu_unpack_backward(
                                              grad_L,
                                              grad_U,
                                              LU_data_sym_argsize_minus_2,
                                              LU_data_sym_argsize_minus_1))
                                        : Tensor();
...
variable_list LuUnpackBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list MaskedFillBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad.masked_fill(mask, 0)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list MaskedFillBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(mask_);
  variable_list result = apply(variable_list(grads));
...
variable_list MaskedFillBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({value_ix})) {
    auto grad_result =
        any_grad_defined ? (masked_fill_backward(grad, mask)) : Tensor();
    copy_range(grad_inputs, value_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list MaskedFillBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(mask_);
  variable_list result = apply(variable_list(grads));
...
variable_list MaskedScatterBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({source_ix})) {
    auto grad_result = any_grad_defined
        ? (masked_scatter_backward_symint(grad, mask, source_sym_sizes))
        : Tensor();
    copy_range(grad_inputs, source_ix, grad_result);
  }
  return grad_inputs;
...
variable_list MaskedScatterBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(mask_);
...
variable_list MaskedScatterBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (grad_output_info.zeros().masked_scatter(mask, grad))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  return grad_inputs;
...
variable_list MaskedScatterBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list MaskedSelectBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (masked_select_backward(grad, self, mask))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list MaskedSelectBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(mask_);
  saved.before(self_);
...
variable_list LinalgMatrixExpBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (linalg_matrix_exp_differential(self, grad, /*adjoint*/ true))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list LinalgMatrixExpBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list MaxBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(keepdim);
...
variable_list MaxBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (evenly_distribute_backward(grad, self, result))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list MaxBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(result_);
...
variable_list MaximumBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    auto grad_result = any_grad_defined
        ? (at::where(self == other, grad / 2, grad)
               .masked_fill_(self > other, 0))
        : Tensor();
    copy_range(grad_inputs, other_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
...
variable_list MaximumBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list FmaxBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    auto grad_result = any_grad_defined
        ? (grad.masked_fill((self >= other).logical_or_(other.isnan()), 0))
        : Tensor();
    copy_range(grad_inputs, other_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
...
variable_list FmaxBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list MeanBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (grad.expand_symint(self_sym_sizes) / self_sym_numel)
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list MeanBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list MeanBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (mean_backward(grad, self_sym_sizes, dim, self_sym_numel, keepdim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list MeanBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(keepdim);
...
variable_list MedianBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(result_);
...
variable_list NanmedianBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(result_);
...
variable_list MedianBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(keepdim);
...
variable_list NanmedianBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(keepdim);
...
variable_list MinBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(keepdim);
...
variable_list MinBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(result_);
...
variable_list MinimumBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list FminBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list AmaxBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (scale_grad_by_count(
              restore_reduced_dims(grad, dim, keepdim),
              restore_reduced_dims(result, dim, keepdim) == self,
              dim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
...
variable_list AmaxBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(keepdim);
...
variable_list AminBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(keepdim);
...
variable_list MmBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({mat2_ix})) {
    auto grad_result = any_grad_defined
        ? (mm_mat2_backward(
              grad, self, mat2_sym_sizes, mat2_sym_strides, mat2_layout, 1))
        : Tensor();
    copy_range(grad_inputs, mat2_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (mm_mat1_backward(
              grad, mat2, self_sym_sizes, self_sym_strides, self_layout, 1))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list MmBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(mat2_);
  saved.before(mat2_layout);
...
variable_list ModeBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(keepdim);
...
variable_list MulBackward0::apply(variable_list&& grads) {
  ...
  auto self = self_.unpack();
  variable_list grad_inputs(2);
  grad_inputs[1] = mul_tensor_backward(grad, self, other_scalar_type);
  grad_inputs[0] = mul_tensor_backward(grad, other, self_scalar_type);
  return grad_inputs;
  /*
  std::lock_guard<std::mutex> lock(mutex_);
  IndexRangeGenerator gen;
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (mul_tensor_backward(grad, other, self_scalar_type))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list MulBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(other_scalar_type);
...
variable_list MulBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other);
  saved.before(self_scalar_type);
...
variable_list MvBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(vec_);
...
variable_list MvlgammaBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (mvlgamma_backward(grad, self, p)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list MvlgammaBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(p);
  saved.before(self_);
...
variable_list NanToNumBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad * at::isfinite(self)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list NanToNumBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list NativeBatchNormBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({input_ix, weight_ix, bias_ix})) {
    ...
    auto grad_result = grad.defined() ? native_batch_norm_backward(
                                            grad,
                                            input,
                                            weight,
                                            running_mean,
...
variable_list NativeBatchNormBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(eps);
  saved.before(input_);
...
variable_list NativeBatchNormLegitBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(eps);
  saved.before(input_);
...
variable_list NativeBatchNormLegitNoTrainingBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void NativeBatchNormLegitNoTrainingBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(eps);
  args.collect(input_);
  args.collect(running_mean_);
...
variable_list NativeBatchNormLegitNoTrainingBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(eps);
  saved.before(input_);
...
variable_list NativeBatchNormLegitBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(eps);
  saved.before(input_);
...
variable_list NativeBatchNormBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({input_ix, weight_ix, grad_out_ix})) {
    ...
    auto grad_result = batchnorm_double_backward(
        input,
        weight,
        grads[0],
        grads[1],
  ...
  if (task_should_compute_output({save_invstd_ix})) {
    auto grad_result =
        not_implemented("native_batch_norm_backward save_invstd");
...
variable_list NativeBatchNormBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(eps);
...
variable_list NativeLayerNormBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({input_ix, weight_ix, bias_ix})) {
    ...
    auto grad_result = grad.defined() ? native_layer_norm_backward_symint(
                                            grad,
                                            input,
                                            normalized_shape,
                                            result1,
...
variable_list NativeLayerNormBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(bias_);
  saved.before(input_);
...
variable_list NativeLayerNormBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({input_ix, weight_ix, grad_out_ix})) {
    ...
    auto grad_result = layer_norm_double_backward(
        input,
        weight,
        grads[0],
        grads[1],
...
variable_list NativeLayerNormBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(grad_out_);
  saved.before(input_);
...
variable_list NativeGroupNormBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({input_ix, weight_ix, bias_ix})) {
    ...
    auto grad_result =
        GradMode::is_enabled() || grads[1].defined() || grads[2].defined()
        ...
        : (grads[0].defined()
               ? native_group_norm_backward_symint(
                     grads[0].device().is_xpu()
                         ? grads[0]
                         : grads[0].contiguous(
                               grads[0].device().is_cpu()
...
variable_list NativeGroupNormBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(C);
  saved.before(HxW);
...
variable_list NeBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_info);
  variable_list result = apply(variable_list(grads));
...
variable_list NeBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_info);
  saved.before(self_info);
...
variable_list NegBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list NextafterBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list NormBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (norm_backward(grad, self, p, result)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list NormBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(p);
  saved.before(self_);
...
variable_list NormBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (norm_backward(grad, self, p, result, dim, keepdim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list NormBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(keepdim);
...
variable_list NormBackward2::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (norm_backward(grad, self.to(grad.scalar_type()), p, result))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list NormBackward2::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(p);
  saved.before(self_);
...
variable_list NormBackward3::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (norm_backward(
              grad, self.to(grad.scalar_type()), p, result, dim, keepdim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list NormBackward3::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(keepdim);
...
variable_list LinalgVectorNormBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (linalg_vector_norm_backward(grad, self, ord, result, dim, keepdim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list LinalgVectorNormBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(keepdim);
...
variable_list PdistBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (_pdist_backward(grad, self, p, result)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list PdistBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(p);
  saved.before(self_);
...
variable_list PdistBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list EuclideanDistBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(x1_);
  saved.before(x2_);
...
variable_list CdistBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({x1_ix})) {
    auto grad_result = any_grad_defined
        ? (_cdist_backward(grad.contiguous(), x1, x2, p, result))
        : Tensor();
  ...
  if (task_should_compute_output({x2_ix})) {
    auto grad_result = any_grad_defined
        ? (_cdist_backward(
              grad.mT().contiguous(), x2, x1, p, result.mT().contiguous()))
        : Tensor();
...
variable_list CdistBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(p);
  saved.before(x1_);
...
variable_list CdistBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list NormalBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list NormalBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({mean_ix})) {
    auto grad_result = any_grad_defined
        ? (at::zeros_symint(mean_sym_sizes, grad.options()))
        : Tensor();
    copy_range(grad_inputs, mean_ix, grad_result);
  }
  return grad_inputs;
...
variable_list NormalBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list NormalBackward2::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({std_ix})) {
    auto grad_result = any_grad_defined
        ? (at::zeros_symint(std_sym_sizes, grad.options()))
        : Tensor();
...
variable_list NormalBackward2::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list NormalBackward3::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(mean_sym_sizes);
  saved.before(std_sym_sizes);
...
variable_list LinalgHouseholderProductBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(input_);
  saved.before(tau_);
...
variable_list OrmqrBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix, input2_ix, input3_ix})) {
    ...
    auto grad_result = ormqr_backward(
        grad, result, self, input2, input3, left, transpose, grad_input_mask);
    if (task_should_compute_output({self_ix})) {
      copy_range(grad_inputs, self_ix, std::get<0>(grad_result));
    }
    if (task_should_compute_output({input2_ix})) {
...
variable_list OrmqrBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(input2_);
...
variable_list PermuteBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (permute_backwards(grad, dims)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list PermuteBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dims);
  variable_list result = apply(variable_list(grads));
...
variable_list PoissonBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_info);
  variable_list result = apply(variable_list(grads));
...
variable_list PowBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (pow_backward(grad, self, exponent)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list PowBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list PowBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({exponent_ix})) {
    auto grad_result = any_grad_defined
        ? (pow_backward_exponent(grad, self, exponent, result))
        : Tensor();
    copy_range(grad_inputs, exponent_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (pow_backward_self(grad, self, exponent)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list PowBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(exponent_);
  saved.before(self_);
...
variable_list PowBackward2::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(exponent_);
  saved.before(self);
...
variable_list ProdBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (prod_backward(grad, self.to(grad.scalar_type()), result))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list ProdBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(result_);
...
variable_list ProdBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (prod_backward(
              grad, self.to(grad.scalar_type()), result, dim, keepdim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list ProdBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(keepdim);
...
variable_list PutBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (accumulate ? grad : grad.put(index, source_info.zeros(), false))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  if (task_should_compute_output({source_ix})) {
    auto grad_result =
        any_grad_defined ? (grad.take(index).reshape_as(source)) : Tensor();
    copy_range(grad_inputs, source_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list PutBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(accumulate);
  saved.before(index_);
...
variable_list LinalgQrBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({A_ix})) {
    auto grad_result = any_grad_defined
        ? (linalg_qr_backward(grad_Q, grad_R, Q, R, mode))
        : Tensor();
    copy_range(grad_inputs, A_ix, grad_result);
  }
  return grad_inputs;
...
variable_list LinalgQrBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(mode);
  saved.before(Q_);
...
variable_list Rad2DegBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list RandomBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list RandomBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list RandomBackward2::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list ReciprocalBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (-grad * (result * result).conj()) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list ReciprocalBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list RemainderBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list RemainderBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    auto grad_result = any_grad_defined
        ? (-grad * self.div(other, /*rounding_mode=*/"floor"))
        : Tensor();
    copy_range(grad_inputs, other_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
...
variable_list RemainderBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list RenormBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (renorm_backward(grad, self, p, dim, maxnorm))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list RenormBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
...
variable_list RepeatBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (repeat_backward(grad, repeats, self_sym_sizes))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list RepeatBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list SpecialEntrBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad * (-(1 + self.log()))) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SpecialEntrBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list SpecialNdtriBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (grad * std::sqrt(2 * M_PI) * (result.square() / 2).exp())
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SpecialNdtriBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list SpecialLogNdtrBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (grad / std::sqrt(2 * M_PI) * (result + self.pow(2) / 2).neg().exp())
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SpecialLogNdtrBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(result_);
...
variable_list ReshapeAliasBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad.reshape_symint(self_sym_sizes)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list ReshapeAliasBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_sym_sizes);
  variable_list result = apply(variable_list(grads));
...
variable_list RoundBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list RoundBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list RsqrtBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (-0.5 * grad * result.pow(3).conj()) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list RsqrtBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list ScatterBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad.scatter(dim, index, 0)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
...
variable_list ScatterBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(index_);
...
variable_list ScatterBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(index_);
...
variable_list ScatterAddBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(index_);
...
variable_list SelectBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (select_backward_symint(grad, self_sym_sizes, dim, index))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SelectBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(index);
...
variable_list SelectBackwardAutogradNestedTensor0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (_nested_select_backward_symint(grad, self, dim, index))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
void SelectBackwardAutogradNestedTensor0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(dim);
  args.collect(index);
  args.collect(self_);
}
variable_list SelectBackwardAutogradNestedTensor0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(index);
...
variable_list SelectBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result =
        any_grad_defined ? (grad.select_symint(dim, index)) : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SelectBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(index);
...
variable_list SigmoidBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (sigmoid_backward(grad, result)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SigmoidBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list LogitBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(eps);
  saved.before(self_);
...
variable_list SignBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list SgnBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (sgn_backward(self, grad, result)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SgnBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(result_);
...
variable_list SinBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list SincBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (sinc_backward(grad, self)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SincBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list SinhBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad * self.cosh().conj()) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SinhBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list SliceBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (slice_backward_wrapper(grad, self_sym_sizes, dim, start, end, step))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SliceBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(end);
...
variable_list SliceBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (grad.slice_symint(dim, start, end, step))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SliceBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(end);
...
variable_list SliceScatterBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (slice_scatter_symint(grad, src_info.zeros(), dim, start, end, step))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  if (task_should_compute_output({src_ix})) {
...
variable_list SliceScatterBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(end);
...
variable_list SelectScatterBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(index);
...
variable_list DiagonalScatterBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (diagonal_scatter(grad, src_info.zeros(), offset, dim1, dim2))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  if (task_should_compute_output({src_ix})) {
...
variable_list DiagonalScatterBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim1);
  saved.before(dim2);
...
variable_list AsStridedScatterBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (as_strided_scatter_backward(
              grad, self_geometry, src_geometry, size, stride, storage_offset))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  if (task_should_compute_output({src_ix})) {
    auto grad_result = any_grad_defined
        ? (grad.contiguous().as_strided_symint(size, stride, storage_offset))
        : Tensor();
    copy_range(grad_inputs, src_ix, grad_result);
  }
  return grad_inputs;
...
variable_list AsStridedScatterBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_geometry);
  saved.before(size);
...
variable_list LinalgSolveExBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({A_ix, B_ix})) {
    ...
    auto grad_result = linalg_solve_backward(
        grad, result, A, LU, pivots, left, grad_input_mask[1]);
    if (task_should_compute_output({A_ix})) {
...
variable_list LinalgSolveExBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(A_);
  saved.before(left);
...
variable_list SortBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (value_selecting_reduction_backward_symint(
              grad, dim, indices, self_sym_sizes, true))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SortBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_sym_sizes);
...
variable_list SortBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_sym_sizes);
...
variable_list SplitBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (split_backward(grads, split_size, dim, self_sym_sizes, self_options))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SplitBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_options);
...
variable_list UnsafeSplitBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_options);
...
variable_list SplitWithSizesBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_options);
...
variable_list SplitWithSizesBackwardAutogradNestedTensor0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (_nested_split_with_sizes_backward(
              grads,
              split_sizes,
              dim,
              at::native::get_nested_tensor_impl(self)->get_nested_sizes(),
              self_options))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
...
void SplitWithSizesBackwardAutogradNestedTensor0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(dim);
  args.collect(self_);
  args.collect(self_options);
...
variable_list SplitWithSizesBackwardAutogradNestedTensor0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_);
...
variable_list UnsafeSplitWithSizesBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_options);
...
variable_list SqrtBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad / (2 * result.conj())) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SqrtBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list SqueezeBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (unsqueeze_to(grad, self_sym_sizes)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SqueezeBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_sym_sizes);
  variable_list result = apply(variable_list(grads));
...
variable_list SqueezeBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (unsqueeze_to(grad, dim, self_sym_sizes)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SqueezeBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_sym_sizes);
...
variable_list SqueezeBackwardAutogradNestedTensor0::apply(
    variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
...
void SqueezeBackwardAutogradNestedTensor0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(dim);
}
variable_list SqueezeBackwardAutogradNestedTensor0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  variable_list result = apply(variable_list(grads));
...
variable_list SqueezeBackward2::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_sym_sizes);
...
variable_list SqueezeBackwardAutogradNestedTensor1::apply(
    variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (unsqueeze_multiple(grad, dim, self_dim)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
void SqueezeBackwardAutogradNestedTensor1::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(dim);
  args.collect(self_dim);
}
variable_list SqueezeBackwardAutogradNestedTensor1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
...
variable_list SqueezeBackward3::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_sym_sizes);
  variable_list result = apply(variable_list(grads));
...
variable_list SqueezeBackward4::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_sym_sizes);
...
variable_list SqueezeBackward5::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_sym_sizes);
...
variable_list StdBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (std_backward(result, grad, self, dim, correction, keepdim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list StdBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list StdMeanBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (std_mean_backward(
              grads[0], grads[1], self, result0, dim, correction, keepdim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list StdMeanBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(correction);
  saved.before(dim);
...
variable_list SubBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    auto grad_result = any_grad_defined
        ? (handle_r_to_c(
              other_scalar_type, maybe_multiply(-grad, alpha.conj())))
        : Tensor();
    copy_range(grad_inputs, other_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
...
variable_list SubBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
  saved.before(other_scalar_type);
...
variable_list SubBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_scalar_type);
  variable_list result = apply(variable_list(grads));
...
variable_list RsubBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    auto grad_result =
        any_grad_defined ? (handle_r_to_c(other_scalar_type, grad)) : Tensor();
    copy_range(grad_inputs, other_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (handle_r_to_c(self_scalar_type, maybe_multiply(-grad, alpha.conj())))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list RsubBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
  saved.before(other_scalar_type);
...
variable_list RsubBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
  saved.before(self_scalar_type);
...
variable_list SumBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad.expand_symint(self_sym_sizes)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SumBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_sym_sizes);
  variable_list result = apply(variable_list(grads));
...
variable_list SumBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (sum_backward(grad, self_sym_sizes, dim, keepdim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SumBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(keepdim);
...
variable_list SumBackwardAutogradNestedTensor0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (_nested_sum_backward(grad, self, dim, keepdim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SumBackwardAutogradNestedTensor0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(keepdim);
...
variable_list NansumBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (nansum_backward(grad.to(self_scalar_type), self, dim, keepdim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list NansumBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(keepdim);
...
variable_list LinalgSvdBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({A_ix})) {
    auto grad_result = any_grad_defined
        ? (svd_backward(
              full_matrices && grad_U.defined()
                  ? grad_U.narrow_symint(-1, 0, S_sym_argsize_minus_1)
                  : grad_U,
              grad_S,
              full_matrices && grad_Vh.defined()
                  ? grad_Vh.narrow_symint(-2, 0, S_sym_argsize_minus_1)
                  : grad_Vh,
              full_matrices ? U.narrow_symint(-1, 0, S_sym_argsize_minus_1) : U,
              S,
              full_matrices ? Vh.narrow_symint(-2, 0, S_sym_argsize_minus_1)
                            : Vh))
        : Tensor();
    copy_range(grad_inputs, A_ix, grad_result);
  }
...
variable_list LinalgSvdBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list LinalgEighBackward0::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (task_should_compute_output({A_ix})) {
    auto grad_result = any_grad_defined ? (linalg_eig_backward(
                                              grads[0],
                                              grads[1],
                                              eigenvalues,
                                              eigenvectors,
...
variable_list LinalgEighBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list LinalgEigBackward0::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (handle_r_to_c(
                                              self_scalar_type,
                                              linalg_eig_backward(
                                                  grads[0],
                                                  grads[1],
...
variable_list LinalgEigBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_scalar_type);
  saved.before(eigenvalues_);
...
variable_list TBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list TBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list FlipBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dims);
  variable_list result = apply(variable_list(grads));
...
variable_list RollBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (grad.roll_symint(
              fmap(
                  reverse_list_symint(shifts),
                  [](c10::SymInt i) { return -i; }),
              reverse_list(dims)))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
...
variable_list RollBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dims);
  saved.before(shifts);
...
variable_list Rot90Backward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dims);
  saved.before(k);
...
variable_list TakeBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (take_backward(grad, self, index)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list TakeBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(index_);
  saved.before(self_);
...
variable_list TanBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad * (1 + result.pow(2)).conj()) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list TanBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list TanhBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (tanh_backward(grad, result)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list TanhBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list TopkBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_sym_sizes);
...
variable_list TraceBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (trace_backward_symint(grad, self_sym_sizes))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list TraceBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_sym_sizes);
  variable_list result = apply(variable_list(grads));
...
variable_list TransposeBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad.transpose(dim0, dim1)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list TransposeBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim0);
  saved.before(dim1);
...
variable_list TransposeBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim0);
  saved.before(dim1);
...
variable_list TriangularSolveBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix, A_ix})) {
    ...
    auto grad_result = triangular_solve_backward(
        grad_solution,
        grad_cloned_coefficient,
        self,
        A,
        solution,
        upper,
...
variable_list TriangularSolveBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(A_);
  saved.before(self_);
...
variable_list LinalgSolveTriangularBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix, B_ix})) {
    ...
    auto grad_result = linalg_solve_triangular_backward(
        grad, self, result, upper, left, unitriangular, grad_input_mask);
    if (task_should_compute_output({self_ix})) {
      copy_range(grad_inputs, self_ix, std::get<0>(grad_result));
    }
    if (task_should_compute_output({B_ix})) {
...
variable_list LinalgSolveTriangularBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(left);
  saved.before(self_);
...
variable_list TrilBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list TriuBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(diagonal);
  variable_list result = apply(variable_list(grads));
...
variable_list TruncBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list ToDenseBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (to_dense_backward(grad, self, masked_grad))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list ToDenseBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list ToSparseBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (to_sparse_backward(grad, self_layout, self_self_sym_blocksize_opt))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list ToSparseBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_layout);
...
variable_list ToSparseBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_layout);
  saved.before(self_self_sym_blocksize_opt);
...
variable_list ToSparseCsrBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_layout);
  saved.before(self_self_sym_blocksize_opt);
...
variable_list ToSparseCscBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_layout);
  saved.before(self_self_sym_blocksize_opt);
...
variable_list ToSparseBsrBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_layout);
  saved.before(self_self_sym_blocksize_opt);
...
variable_list ToSparseBscBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_layout);
  saved.before(self_self_sym_blocksize_opt);
...
variable_list ToMkldnnBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (to_mkldnn_backward(grad, self)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list ToMkldnnBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list UnfoldBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (unfold_backward_symint(grad, self_sym_sizes, dimension, size, step))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list UnfoldBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list UnfoldBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_in_ix})) {
    auto grad_result =
        any_grad_defined ? (grad.unfold(dim, size, step)) : Tensor();
...
variable_list UnfoldBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(size);
...
variable_list UniformBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list UniqueBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list UniqueDimBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list UniqueConsecutiveBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list UniqueDimConsecutiveBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list Unique2Backward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list UnsafeViewBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_sym_sizes);
  variable_list result = apply(variable_list(grads));
...
variable_list LiftBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list LiftFreshBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list UnsqueezeBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  variable_list result = apply(variable_list(grads));
...
variable_list UnsqueezeBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  variable_list result = apply(variable_list(grads));
...
variable_list VarBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(correction);
  saved.before(dim);
...
variable_list VarMeanBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (var_mean_backward(
              grads[0], grads[1], self, dim, correction, keepdim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list VarMeanBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(correction);
  saved.before(dim);
...
variable_list ViewBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_sym_sizes);
  variable_list result = apply(variable_list(grads));
...
variable_list ViewBackwardAutogradNestedTensor0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ViewAsRealBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (at::view_as_complex(grad.contiguous())) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
void ViewAsRealBackward0::compiled_args(CompiledNodeArgs& args) {}
variable_list ViewAsRealBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list ViewAsComplexBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (at::view_as_real(grad.contiguous().resolve_conj()))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list ViewAsComplexBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list WhereBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    auto grad_result =
        any_grad_defined ? (where(condition, 0, grad)) : Tensor();
    copy_range(grad_inputs, other_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (where(condition, grad, 0)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list WhereBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list WeightNormInterfaceBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({v_ix, g_ix})) {
    auto grad_result = grad.defined()
        ? (GradMode::is_enabled() ? _weight_norm_differentiable_backward(
                                        grad.contiguous(), v, g, result1, dim)
...
variable_list WeightNormInterfaceBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(g_);
...
variable_list ZeroBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list SparseMaskBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (sparse_mask_backward(grad, mask, self_layout))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SparseMaskBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(mask_);
  saved.before(self_layout);
...
variable_list SparseCooTensorWithDimsAndTensorsBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (task_should_compute_output({values_ix})) {
    auto grad_result =
        any_grad_defined ? (grad.sparse_mask(result)._values()) : Tensor();
    copy_range(grad_inputs, values_ix, grad_result);
  }
  return grad_inputs;
}
void SparseCooTensorWithDimsAndTensorsBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(result_);
}
variable_list SparseCooTensorWithDimsAndTensorsBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list SparseCompressedTensorBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({values_ix})) {
    auto grad_result = any_grad_defined
        ? (grad.to_dense().sparse_mask(result).values())
        : Tensor();
    copy_range(grad_inputs, values_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SparseCompressedTensorBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list SparseSumBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (at::_sparse_sum_backward(grad, self, dim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SparseSumBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_);
...
variable_list StandardGammaBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (grad * _standard_gamma_grad(self, result))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list StandardGammaBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(result_);
...
variable_list StandardGammaGradBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list ValuesBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (at::_sparse_coo_tensor_unsafe_symint(
               self.indices(), grad, self_sym_sizes)
               ._coalesced_(true))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
...
variable_list ValuesBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(self_sym_sizes);
...
variable_list ValuesBackwardAutogradNestedTensor0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void ValuesBackwardAutogradNestedTensor0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(self_);
}
variable_list ValuesBackwardAutogradNestedTensor0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list TrilinearBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({i1_ix, i2_ix, i3_ix})) {
    ...
    auto grad_result = _trilinear_backward(
        grad,
...
variable_list TrilinearBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list ConstantPadNdBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (constant_pad_nd_backward(grad, pad)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list ConstantPadNdBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(pad);
  variable_list result = apply(variable_list(grads));
...
variable_list BinaryCrossEntropyBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (binary_cross_entropy_backward(grad, self, target, weight, reduction))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  if (task_should_compute_output({target_ix})) {
    auto grad_result = any_grad_defined
        ? (binary_cross_entropy_target_backward(
              grad, self, target, weight, reduction))
        : Tensor();
...
variable_list BinaryCrossEntropyBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list BinaryCrossEntropyBackwardBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (binary_cross_entropy_double_backward_grad_output(
              grad, self, target, weight, reduction))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (binary_cross_entropy_double_backward(
              grad_output, grad, self, target, weight, reduction))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  if (task_should_compute_output({target_ix})) {
    auto grad_result = any_grad_defined
        ? (binary_cross_entropy_double_backward_target(
              grad, grad_output, self, target, weight, reduction))
        : Tensor();
    copy_range(grad_inputs, target_ix, grad_result);
  }
  return grad_inputs;
}
void BinaryCrossEntropyBackwardBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(grad_output_);
  args.collect(reduction);
  args.collect(self_);
...
variable_list BinaryCrossEntropyBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(grad_output_);
  saved.before(reduction);
...
variable_list BinaryCrossEntropyWithLogitsBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (binary_cross_entropy_with_logits_backward(
              grad, self, target, weight, pos_weight, reduction))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  if (task_should_compute_output({target_ix})) {
    auto grad_result = any_grad_defined
        ? (binary_cross_entropy_with_logits_target_backward(
              grad, self, target, weight, pos_weight, reduction))
        : Tensor();
    copy_range(grad_inputs, target_ix, grad_result);
  }
...
void BinaryCrossEntropyWithLogitsBackward0::compiled_args(
    CompiledNodeArgs& args) {
...
variable_list BinaryCrossEntropyWithLogitsBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list EmbeddingBackward0::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (task_should_compute_output({weight_ix})) {
    auto grad_result = any_grad_defined ? (embedding_backward_symint(
                                              grad,
                                              indices,
                                              weight_sym_argsize_0,
                                              padding_idx,
                                              scale_grad_by_freq,
                                              sparse))
                                        : Tensor();
...
variable_list EmbeddingBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(indices_);
...
variable_list EmbeddingDenseBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (embedding_dense_double_backward_symint(grad, indices, padding_idx))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  return grad_inputs;
...
variable_list EmbeddingDenseBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(indices_);
  saved.before(padding_idx);
  ...
  if (task_should_compute_output({per_sample_weights_ix})) {
    auto grad_result = any_grad_defined
        ? (_embedding_bag_per_sample_weights_backward(
              grad, weight, indices, offsets, result1, mode, padding_idx))
        : Tensor();
    copy_range(grad_inputs, per_sample_weights_ix, grad_result);
  }
  if (task_should_compute_output({weight_ix})) {
    auto grad_result = any_grad_defined ? (_embedding_bag_backward_symint(
                                              grad,
                                              indices,
                                              offsets,
                                              result1,
...
variable_list EmbeddingBagBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(indices_);
  saved.before(mode);
...
variable_list EmbeddingRenormBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list MseLossBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (mse_loss_backward(grad, self, target, reduction))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  if (task_should_compute_output({target_ix})) {
    auto grad_result = any_grad_defined
        ? (mse_loss_backward(grad, target, self, reduction))
        : Tensor();
    copy_range(grad_inputs, target_ix, grad_result);
  }
  return grad_inputs;
...
variable_list MseLossBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(reduction);
  saved.before(self_);
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (multi_margin_loss_backward(
              grad, self, target, p, margin, weight, reduction))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list MultiMarginLossBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(margin);
  saved.before(p);
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (multilabel_margin_loss_backward(
              grad, self, target, reduction, is_target))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list MultilabelMarginLossBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(reduction);
  saved.before(self_);
...
variable_list NllLossBackward0::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (nll_loss_backward_symint(
                                              grad,
                                              self,
                                              target,
                                              weight,
...
variable_list NllLossBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list NllLoss2DBackward0::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (nll_loss2d_backward_symint(
                                              grad,
                                              self,
                                              target,
                                              weight,
...
variable_list NllLoss2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(ignore_index);
  saved.before(reduction);
...
variable_list SmoothL1LossBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (smooth_l1_loss_backward(grad, self, target, reduction, beta))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  if (task_should_compute_output({target_ix})) {
    auto grad_result = any_grad_defined
        ? (smooth_l1_loss_backward(grad, target, self, reduction, beta))
        : Tensor();
    copy_range(grad_inputs, target_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SmoothL1LossBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(beta);
  saved.before(reduction);
...
variable_list HuberLossBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (huber_loss_backward(grad, self, target, reduction, delta))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  if (task_should_compute_output({target_ix})) {
    auto grad_result = any_grad_defined
        ? (huber_loss_backward(grad, target, self, reduction, delta))
        : Tensor();
    copy_range(grad_inputs, target_ix, grad_result);
  }
  return grad_inputs;
...
variable_list HuberLossBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(delta);
  saved.before(reduction);
...
variable_list SoftMarginLossBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (soft_margin_loss_backward(grad, self, target, reduction))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SoftMarginLossBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(reduction);
  saved.before(self_);
...
variable_list ReluBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (threshold_backward(grad, result, 0)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list ReluBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list SiluBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (GradMode::is_enabled()
               ? infinitely_differentiable_silu_backward(grad, self)
               : silu_backward(grad, self))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SiluBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list MishBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (GradMode::is_enabled()
               ? infinitely_differentiable_mish_backward(grad, self)
               : mish_backward(grad, self))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list MishBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list EluBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (elu_backward(
              grad, alpha, scale, input_scale, /* is_result */ false, self))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list EluBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
...
variable_list EluBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (elu_backward(
              grad, alpha, scale, input_scale, /* is_result */ true, result))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list EluBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
  saved.before(input_scale);
...
variable_list CeluBackward0::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (elu_backward(
                                              grad,
                                              alpha,
                                              1,
                                              1.0 / alpha.toFloat(),
...
variable_list CeluBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
  saved.before(self_);
...
variable_list CeluBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
  saved.before(result_);
...
variable_list GeluBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (gelu_backward(grad, self, approximate)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list GeluBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list GeluBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (gelu_double_backward(grad, grad_output, self, approximate))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list GeluBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(approximate);
  saved.before(grad_output_);
...
variable_list GluBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (glu_backward(grad, self, dim)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list GluBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_);
...
variable_list HardshrinkBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (hardshrink_backward(grad, self, lambd)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list HardshrinkBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(lambd);
  saved.before(self_);
...
variable_list HardshrinkBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(lambd);
  saved.before(self_);
...
variable_list HardtanhBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (hardtanh_backward(grad, self, min_val, max_val))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list HardtanhBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list LeakyReluBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (leaky_relu_backward(grad, self, negative_slope, false))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list LeakyReluBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list LeakyReluBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (leaky_relu_backward(grad, result, negative_slope, true))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list LeakyReluBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(negative_slope);
  saved.before(result_);
...
variable_list LogSigmoidBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (log_sigmoid_backward(grad, self, buffer))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list LogSigmoidBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
...
variable_list LogSoftmaxBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (_log_softmax_backward_data(grad, result, dim, self_scalar_type))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list LogSoftmaxBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_scalar_type);
...
variable_list SparseLogSoftmaxBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (_sparse_log_softmax_backward_data(grad, result, dim, self))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SparseLogSoftmaxBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_);
...
variable_list MaskedSoftmaxBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (_masked_softmax_backward(grad, result, mask, dim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list MaskedSoftmaxBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(mask_);
...
variable_list PreluKernelBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix, weight_ix})) {
    auto grad_result = grad.defined()
        ? _prelu_kernel_backward(grad, self, weight)
        : std::tuple<Tensor, Tensor>();
    if (task_should_compute_output({self_ix})) {
      copy_range(grad_inputs, self_ix, std::get<0>(grad_result));
    }
...
variable_list PreluKernelBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(weight_);
...
variable_list PreluKernelBackwardBackward0::apply(variable_list&& grads) {
        ...
        ? (grads[0].defined()
               ? (grads[1].defined()
                      ? at::where(
                            self >= 0,
                            grads[0],
                            grads[0] * weight + grads[1] * self)
                      : at::where(self >= 0, grads[0], grads[0] * weight))
               : at::where(
                     self >= 0,
                     at::zeros({}, grad_output_options),
                     grads[1] * self))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  ...
  if (task_should_compute_output({weight_ix})) {
    auto grad_result = any_grad_defined
        ? (grads[0].defined() ? at::where(
                                    self >= 0,
                                    at::zeros({}, weight_options),
                                    grad_output * grads[0])
                              : self_info.zeros())
        : Tensor();
    copy_range(grad_inputs, weight_ix, grad_result);
...
variable_list PreluKernelBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(grad_output_);
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (rrelu_with_noise_backward(
              grad, self, noise, lower, upper, training, false))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list RreluWithNoiseBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(lower);
  saved.before(noise_);
...
variable_list RreluWithNoiseBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (rrelu_with_noise_backward(
              grad, result, noise, lower, upper, training, true))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list RreluWithNoiseBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(lower);
  saved.before(noise_);
...
variable_list SoftmaxBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (_softmax_backward_data(grad, result, dim, self_scalar_type))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SoftmaxBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_scalar_type);
...
variable_list SparseSoftmaxBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (_sparse_softmax_backward_data(grad, result, dim, self))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SparseSoftmaxBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_);
...
variable_list SparseSparseMatmulBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    auto grad_result = any_grad_defined
        ? (sparse_sparse_matmul_backward(grad, self, other, 1))
        : Tensor();
    copy_range(grad_inputs, other_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (sparse_sparse_matmul_backward(grad, self, other, 0))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SparseSparseMatmulBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list SoftplusBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (softplus_backward(grad, self, beta, threshold))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SoftplusBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(beta);
  saved.before(self_);
...
variable_list SoftshrinkBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (softshrink_backward(grad, self, lambd)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SoftshrinkBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(lambd);
  saved.before(self_);
...
variable_list ThresholdBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (threshold_backward(grad, self, threshold))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list ThresholdBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(threshold);
...
variable_list ThresholdBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(threshold);
...
variable_list ReflectionPad1DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (reflection_pad1d_backward_symint(grad, self, padding))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list ReflectionPad1DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list ReflectionPad2DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (reflection_pad2d_backward_symint(grad, self, padding))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list ReflectionPad2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(padding);
  saved.before(self_);
...
variable_list ReflectionPad3DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (reflection_pad3d_backward_symint(grad, self, padding))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list ReflectionPad3DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(padding);
  saved.before(self_);
...
variable_list ReplicationPad1DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (replication_pad1d_backward_symint(grad, self, padding))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list ReplicationPad1DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(padding);
  saved.before(self_);
...
variable_list ReplicationPad2DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (replication_pad2d_backward_symint(grad, self, padding))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list ReplicationPad2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(padding);
  saved.before(self_);
...
variable_list ReplicationPad3DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (replication_pad3d_backward_symint(grad, self, padding))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list ReplicationPad3DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(padding);
  saved.before(self_);
...
variable_list UpsampleLinear1DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (upsample_linear1d_backward_symint(
              grad, output_size, self_sym_sizes, align_corners, scales))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list UpsampleLinear1DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(align_corners);
...
variable_list UpsampleBilinear2DBackward0::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (upsample_bilinear2d_backward_symint(
                                              grad,
                                              output_size,
                                              self_sym_sizes,
                                              align_corners,
...
variable_list UpsampleBilinear2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(align_corners);
  saved.before(output_size);
...
variable_list UpsampleBilinear2DAaBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (_upsample_bilinear2d_aa_backward_symint(
              grad,
              output_size,
              self_sym_sizes,
              align_corners,
...
variable_list UpsampleBilinear2DAaBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(align_corners);
  saved.before(output_size);
...
variable_list UpsampleBicubic2DBackward0::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (upsample_bicubic2d_backward_symint(
                                              grad,
                                              output_size,
                                              self_sym_sizes,
                                              align_corners,
...
variable_list UpsampleBicubic2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(align_corners);
  saved.before(output_size);
...
variable_list UpsampleBicubic2DAaBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (_upsample_bicubic2d_aa_backward_symint(
              grad,
              output_size,
              self_sym_sizes,
              align_corners,
...
variable_list UpsampleBicubic2DAaBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(align_corners);
  saved.before(output_size);
...
variable_list UpsampleTrilinear3DBackward0::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (upsample_trilinear3d_backward_symint(
                                              grad,
                                              output_size,
                                              self_sym_sizes,
                                              align_corners,
...
variable_list UpsampleTrilinear3DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(align_corners);
  saved.before(output_size);
...
variable_list UpsampleNearest1DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (upsample_nearest1d_backward_symint(
              grad, output_size, self_sym_sizes, scales))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list UpsampleNearest1DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(output_size);
  saved.before(scales);
...
variable_list UpsampleNearestExact1DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (_upsample_nearest_exact1d_backward_symint(
              grad, output_size, self_sym_sizes, scales))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
...
variable_list UpsampleNearestExact1DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(output_size);
  saved.before(scales);
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (upsample_nearest2d_backward_symint(
              grad, output_size, self_sym_sizes, scales_h, scales_w))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list UpsampleNearest2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(output_size);
  saved.before(scales_h);
...
variable_list UpsampleNearestExact2DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (_upsample_nearest_exact2d_backward_symint(
              grad, output_size, self_sym_sizes, scales_h, scales_w))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
...
variable_list UpsampleNearestExact2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(output_size);
  saved.before(scales_h);
...
variable_list UpsampleNearest3DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (upsample_nearest3d_backward_symint(
              grad, output_size, self_sym_sizes, scales_d, scales_h, scales_w))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list UpsampleNearest3DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(output_size);
  saved.before(scales_d);
...
variable_list UpsampleNearestExact3DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (_upsample_nearest_exact3d_backward_symint(
              grad, output_size, self_sym_sizes, scales_d, scales_h, scales_w))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
...
variable_list UpsampleNearestExact3DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(output_size);
  saved.before(scales_d);
...
variable_list PixelShuffleBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (pixel_unshuffle(grad, upscale_factor)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list PixelShuffleBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list PixelUnshuffleBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (pixel_shuffle(grad, downscale_factor)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list PixelUnshuffleBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list AdaptiveAvgPool2DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (_adaptive_avg_pool2d_backward(grad, self))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list AdaptiveAvgPool2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list AdaptiveAvgPool3DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (_adaptive_avg_pool3d_backward(grad, self))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list AdaptiveAvgPool3DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list AdaptiveMaxPool2DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (adaptive_max_pool2d_backward(grad, self, result1))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list AdaptiveMaxPool2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(result1_);
...
variable_list AdaptiveMaxPool3DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (adaptive_max_pool3d_backward(grad, self, result1))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list AdaptiveMaxPool3DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(result1_);
...
variable_list AvgPool2DBackward0::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (avg_pool2d_backward(
                                              grad,
                                              self,
                                              kernel_size,
                                              stride,
...
variable_list AvgPool2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list AvgPool3DBackward0::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (avg_pool3d_backward(
                                              grad,
                                              self,
                                              kernel_size,
                                              stride,
...
variable_list AvgPool3DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(ceil_mode);
  saved.before(count_include_pad);
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (fractional_max_pool2d_backward(
              grad, self, kernel_size, output_size, result1))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list FractionalMaxPool2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(kernel_size);
  saved.before(output_size);
...
variable_list FractionalMaxPool3DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(kernel_size);
  saved.before(output_size);
...
variable_list LinearBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({input_ix, weight_ix, bias_ix})) {
    ...
    auto grad_result = grad.defined()
        ? linear_backward(input, grad, weight, grad_input_mask)
        : std::tuple<Tensor, Tensor, Tensor>();
    if (task_should_compute_output({input_ix})) {
      copy_range(grad_inputs, input_ix, std::get<0>(grad_result));
    }
...
variable_list LinearBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(input_);
  saved.before(weight_);
...
variable_list LinearBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(grad_output_);
  saved.before(self_);
...
variable_list MaxPool2DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (max_pool2d_backward(
              grad, self, kernel_size, stride, padding, dilation, ceil_mode))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list MaxPool2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(ceil_mode);
...
variable_list MpsConvolutionBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix, weight_ix, bias_ix})) {
    ...
    auto grad_result = grad.defined() ? mps_convolution_backward_symint(
                                            self,
                                            grad,
                                            weight,
                                            padding,
...
variable_list MpsConvolutionBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dilation);
  saved.before(groups);
...
variable_list MpsConvolutionBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix, self_ix, weight_ix})) {
    ...
    auto grad_result = _convolution_double_backward_symint(
        grads[0],
        grads[1],
        grads[2],
        grad_output,
        ...
        dilation,
        false,
        std::vector<c10::SymInt>(padding.size(), 0),
        groups,
        grad_input_mask);
    if (task_should_compute_output({grad_output_ix})) {
...
variable_list MpsConvolutionBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dilation);
  saved.before(grad_output_);
...
variable_list MaxPool2DWithIndicesBackward0::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (max_pool2d_with_indices_backward(
                                              grad,
                                              self,
                                              kernel_size,
                                              stride,
...
variable_list MaxPool2DWithIndicesBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(ceil_mode);
  saved.before(dilation);
...
variable_list MaxPool3DWithIndicesBackward0::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (max_pool3d_with_indices_backward(
                                              grad,
                                              self,
                                              kernel_size,
                                              stride,
...
variable_list MaxPool3DWithIndicesBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(ceil_mode);
  saved.before(dilation);
...
variable_list MaxUnpool2DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (max_pool_double_backward(grad, indices, 2))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list MaxUnpool2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(indices_);
  variable_list result = apply(variable_list(grads));
...
variable_list MaxUnpool3DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (max_pool_double_backward(grad, indices, 3))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list MaxUnpool3DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(indices_);
  variable_list result = apply(variable_list(grads));
...
variable_list ConvolutionBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({input_ix, weight_ix, bias_ix})) {
    ...
    auto grad_result = grad.defined() ? convolution_backward_symint(
                                            grad,
                                            input,
                                            weight,
                                            bias_sym_sizes_opt,
...
variable_list ConvolutionBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list ConvolutionBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(bias_sym_sizes_opt);
  saved.before(dilation);
...
variable_list ConvolutionBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dilation);
  saved.before(grad_output_);
...
variable_list ConvolutionOverrideableBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({input_ix, weight_ix, bias_ix})) {
    ...
    auto grad_result = grad.defined()
        ? convolution_backward_overrideable_symint(
              grad,
              input,
              weight,
              stride,
...
variable_list ConvolutionOverrideableBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dilation);
  saved.before(groups);
...
variable_list ConvolutionBackwardOverrideableBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void ConvolutionBackwardOverrideableBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(dilation);
  args.collect(grad_output_);
  args.collect(groups);
...
variable_list ConvolutionBackwardOverrideableBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dilation);
  saved.before(grad_output_);
...
variable_list SlowConvTranspose2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(bias_sym_sizes_opt);
  saved.before(dilation);
...
variable_list SlowConvTranspose3DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(bias_sym_sizes_opt);
  saved.before(dilation);
...
variable_list SlowConv2DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix, weight_ix, bias_ix})) {
    ...
    auto grad_result = grad.defined()
        ? _slow_conv2d_backward_symint(
              grad, self, weight, kernel_size, stride, padding, grad_input_mask)
        : std::tuple<Tensor, Tensor, Tensor>();
    if (task_should_compute_output({self_ix})) {
      copy_range(grad_inputs, self_ix, std::get<0>(grad_result));
    }
...
variable_list SlowConv2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(kernel_size);
  saved.before(padding);
...
variable_list SlowConv2DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(grad_output_);
  saved.before(padding);
...
variable_list ConvDepthwise2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(bias_sym_sizes_opt);
  saved.before(dilation);
...
variable_list ConvDepthwise3DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(bias_sym_sizes_opt);
  saved.before(dilation);
...
variable_list SlowConv3DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(bias_sym_sizes_opt);
  saved.before(padding);
...
variable_list SlowConvDilated2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(bias_sym_sizes_opt);
  saved.before(dilation);
...
variable_list SlowConvDilated3DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(bias_sym_sizes_opt);
  saved.before(dilation);
...
variable_list Col2ImBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (im2col(grad, kernel_size, dilation, padding, stride))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list Col2ImBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dilation);
  saved.before(kernel_size);
...
variable_list Im2ColBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (col2im_symint(
              grad,
              {self_sym_argsize_minus_2, self_sym_argsize_minus_1},
              kernel_size,
              dilation,
              padding,
              stride))
...
variable_list Im2ColBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dilation);
  saved.before(kernel_size);
...
variable_list AdaptiveAvgPool2DBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (_adaptive_avg_pool2d_symint(
              grad,
              {grad_output_sym_argsize_minus_2,
               grad_output_sym_argsize_minus_1}))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
...
variable_list AdaptiveAvgPool2DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list AdaptiveAvgPool3DBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (_adaptive_avg_pool3d_symint(
              grad,
              {grad_output_sym_argsize_minus_3,
               grad_output_sym_argsize_minus_2,
               grad_output_sym_argsize_minus_1}))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
...
variable_list AdaptiveAvgPool3DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(grad_output_sym_argsize_minus_1);
  saved.before(grad_output_sym_argsize_minus_2);
...
variable_list AdaptiveMaxPool2DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(indices_);
  saved.before(self_info);
...
variable_list AdaptiveMaxPool3DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(indices_);
  saved.before(self_info);
...
variable_list AvgPool2DBackwardBackward0::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined ? (avg_pool2d(
                                              grad,
                                              kernel_size,
                                              stride,
                                              padding,
...
variable_list AvgPool2DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(ceil_mode);
  saved.before(count_include_pad);
...
variable_list AvgPool3DBackwardBackward0::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined ? (avg_pool3d(
                                              grad,
                                              kernel_size,
                                              stride,
                                              padding,
...
variable_list AvgPool3DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(ceil_mode);
  saved.before(count_include_pad);
...
variable_list EluBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (elu_backward(
              grad, alpha, scale, input_scale, is_result, self_or_result))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (task_should_compute_output({self_or_result_ix})) {
    auto grad_result = any_grad_defined ? (elu_double_backward(
                                              grad,
                                              grad_output,
                                              alpha,
                                              scale,
...
variable_list EluBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
  saved.before(grad_output_);
...
variable_list FractionalMaxPool2DBackwardBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void FractionalMaxPool2DBackwardBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(indices_);
  args.collect(self_info);
}
variable_list FractionalMaxPool2DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(indices_);
  saved.before(self_info);
...
variable_list FractionalMaxPool3DBackwardBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void FractionalMaxPool3DBackwardBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(indices_);
  args.collect(self_info);
}
variable_list FractionalMaxPool3DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(indices_);
  saved.before(self_info);
...
variable_list GluBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (glu_double_backward_grad_output(grad, self, dim))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (glu_double_backward(grad, grad_output, self, dim))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list GluBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(grad_output_);
...
variable_list HardtanhBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(max_val);
  saved.before(min_val);
...
variable_list LogSigmoidBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (log_sigmoid_double_backward(grad * grad_output, self))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list LogSigmoidBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(buffer_);
  saved.before(grad_output_);
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (grad.to(output.dtype()) -
           (grad.to(output.dtype()) * output.exp()).sum(dim, true))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (task_should_compute_output({output_ix})) {
    auto grad_result = any_grad_defined
        ? ((-grad_output.sum(dim, true) * output.exp() *
            grad.to(output.dtype()))
               .to(output.dtype()))
        : Tensor();
...
variable_list LogSoftmaxBackwardDataBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(grad_output_);
...
variable_list LeakyReluBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(negative_slope);
  saved.before(self_);
...
variable_list MaxPool2DBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result =
        any_grad_defined ? (error_for_max_pool2d_double_backward()) : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (self_info.zeros()) : Tensor();
...
variable_list MaxPool2DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_info);
  variable_list result = apply(variable_list(grads));
...
variable_list MaxPool2DWithIndicesBackwardBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void MaxPool2DWithIndicesBackwardBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(indices_);
  args.collect(self_info);
}
variable_list MaxPool2DWithIndicesBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(indices_);
  saved.before(self_info);
...
variable_list MaxPool3DWithIndicesBackwardBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void MaxPool3DWithIndicesBackwardBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(indices_);
  args.collect(self_info);
}
variable_list MaxPool3DWithIndicesBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(indices_);
  saved.before(self_info);
...
variable_list MseLossBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (mse_loss_double_backward(grad * grad_output, self, reduction))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  if (task_should_compute_output({target_ix})) {
    auto grad_result = any_grad_defined
        ? (-mse_loss_double_backward(grad * grad_output, target, reduction))
        : Tensor();
    copy_range(grad_inputs, target_ix, grad_result);
  }
  return grad_inputs;
...
variable_list MseLossBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(grad_output_);
  saved.before(reduction);
...
variable_list NllLossBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (nll_loss_symint(grad, target, weight, reduction, ignore_index))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
...
variable_list NllLossBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(ignore_index);
  saved.before(reduction);
...
variable_list NllLoss2DBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (nll_loss2d_symint(grad, target, weight, reduction, ignore_index))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
...
variable_list NllLoss2DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(ignore_index);
  saved.before(reduction);
...
variable_list RreluWithNoiseBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(lower);
  saved.before(noise_);
...
variable_list ReflectionPad1DBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result =
        any_grad_defined ? (reflection_pad1d_symint(grad, padding)) : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (self_info.zeros()) : Tensor();
...
variable_list ReflectionPad1DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(padding);
  saved.before(self_info);
...
variable_list ReflectionPad2DBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result =
        any_grad_defined ? (reflection_pad2d_symint(grad, padding)) : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (self_info.zeros()) : Tensor();
...
variable_list ReflectionPad2DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(padding);
  saved.before(self_info);
...
variable_list ReflectionPad3DBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result =
        any_grad_defined ? (reflection_pad3d_symint(grad, padding)) : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (self_info.zeros()) : Tensor();
...
variable_list ReflectionPad3DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(padding);
  saved.before(self_info);
...
variable_list ReplicationPad1DBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result =
        any_grad_defined ? (replication_pad1d_symint(grad, padding)) : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (self_info.zeros()) : Tensor();
...
variable_list ReplicationPad1DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(padding);
  saved.before(self_info);
...
variable_list ReplicationPad2DBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result =
        any_grad_defined ? (replication_pad2d_symint(grad, padding)) : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (self_info.zeros()) : Tensor();
...
variable_list ReplicationPad2DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(padding);
  saved.before(self_info);
...
variable_list ReplicationPad3DBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result =
        any_grad_defined ? (replication_pad3d_symint(grad, padding)) : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (self_info.zeros()) : Tensor();
...
variable_list ReplicationPad3DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(padding);
  saved.before(self_info);
...
variable_list SparseSampledAddmmBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix, mat1_ix, mat2_ix})) {
    ...
    auto grad_result = sparse_sampled_addmm_backward(
        grad,
        self,
...
variable_list SparseSampledAddmmBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
  saved.before(beta);
  ...
  if (task_should_compute_output({self_ix, other_ix})) {
    ...
    auto grad_result = grad.defined()
        ? _sparse_mm_reduce_impl_backward(
              self, grad, other, reduce, result1, grad_input_mask)
        : std::tuple<Tensor, Tensor>();
    if (task_should_compute_output({self_ix})) {
      copy_range(grad_inputs, self_ix, std::get<0>(grad_result));
    }
...
variable_list SparseMmReduceImplBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(reduce);
...
variable_list SmoothL1LossBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (smooth_l1_loss_double_backward(
              grad * grad_output, self, target, reduction, beta))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  if (task_should_compute_output({target_ix})) {
...
variable_list SmoothL1LossBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(beta);
  saved.before(grad_output_);
...
variable_list HuberLossBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (huber_loss_double_backward_grad_output(
              grad, grad_output, self, target, reduction, delta))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (task_should_compute_output({self_ix})) {
...
variable_list HuberLossBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(delta);
  saved.before(grad_output_);
...
variable_list SoftplusBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (softplus_double_backward(grad * grad_output, self, beta, threshold))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SoftplusBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(beta);
  saved.before(grad_output_);
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (_softmax_backward_data(
              grad.to(output.dtype()), output, dim, input_dtype))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (task_should_compute_output({output_ix})) {
    auto grad_result = any_grad_defined
        ? (softmax_double_backward(
               grad.to(output.dtype()), grad_output, dim, output)
               .to(output.dtype()))
        : Tensor();
    copy_range(grad_inputs, output_ix, grad_result);
  }
...
variable_list SoftmaxBackwardDataBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(grad_output_);
...
variable_list SoftMarginLossBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (soft_margin_loss_double_backward_grad_output(
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (soft_margin_loss_double_backward(
              grad * grad_output, self, target, reduction))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
...
variable_list SoftMarginLossBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(grad_output_);
  saved.before(reduction);
...
variable_list SoftshrinkBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(lambd);
  saved.before(self_);
...
variable_list ThresholdBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(threshold);
...
variable_list UpsampleLinear1DBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (upsample_linear1d_symint(grad, output_size, align_corners, scales))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  return grad_inputs;
...
variable_list UpsampleLinear1DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(align_corners);
  saved.before(output_size);
...
variable_list UpsampleBilinear2DBackwardBackward0::apply(
    variable_list&& grads) {
  IndexRangeGenerator gen;
  auto grad_output_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (upsample_bilinear2d_symint(
              grad, output_size, align_corners, scales_h, scales_w))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  return grad_inputs;
}
void UpsampleBilinear2DBackwardBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(align_corners);
  args.collect(output_size);
  args.collect(scales_h);
...
variable_list UpsampleBilinear2DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(align_corners);
  saved.before(output_size);
...
variable_list UpsampleBilinear2DAaBackwardBackward0::apply(
    variable_list&& grads) {
  IndexRangeGenerator gen;
  auto grad_output_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (_upsample_bilinear2d_aa_symint(
              grad, output_size, align_corners, scales_h, scales_w))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
...
void UpsampleBilinear2DAaBackwardBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(align_corners);
  args.collect(output_size);
  args.collect(scales_h);
...
variable_list UpsampleBilinear2DAaBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(align_corners);
  saved.before(output_size);
...
variable_list UpsampleBicubic2DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(align_corners);
  saved.before(output_size);
...
variable_list UpsampleBicubic2DAaBackwardBackward0::apply(
    variable_list&& grads) {
  IndexRangeGenerator gen;
  auto grad_output_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
...
void UpsampleBicubic2DAaBackwardBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(align_corners);
  args.collect(output_size);
  args.collect(scales_h);
...
variable_list UpsampleBicubic2DAaBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(align_corners);
  saved.before(output_size);
...
variable_list UpsampleTrilinear3DBackwardBackward0::apply(
    variable_list&& grads) {
  IndexRangeGenerator gen;
  auto grad_output_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (upsample_trilinear3d_symint(
              grad, output_size, align_corners, scales_d, scales_h, scales_w))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  return grad_inputs;
}
void UpsampleTrilinear3DBackwardBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(align_corners);
  args.collect(output_size);
  args.collect(scales_d);
...
variable_list UpsampleTrilinear3DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(align_corners);
  saved.before(output_size);
...
variable_list UpsampleNearest1DBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (upsample_nearest1d_symint(grad, output_size, scales))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  return grad_inputs;
...
variable_list UpsampleNearest1DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(output_size);
  saved.before(scales);
...
variable_list UpsampleNearestExact1DBackwardBackward0::apply(
    variable_list&& grads) {
  IndexRangeGenerator gen;
  auto grad_output_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (_upsample_nearest_exact1d_symint(grad, output_size, scales))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  return grad_inputs;
}
void UpsampleNearestExact1DBackwardBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(output_size);
  args.collect(scales);
}
variable_list UpsampleNearestExact1DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(output_size);
  saved.before(scales);
...
variable_list UpsampleNearest2DBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (upsample_nearest2d_symint(grad, output_size, scales_h, scales_w))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  return grad_inputs;
...
variable_list UpsampleNearest2DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(output_size);
  saved.before(scales_h);
...
variable_list UpsampleNearestExact2DBackwardBackward0::apply(
    variable_list&& grads) {
  IndexRangeGenerator gen;
  auto grad_output_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (_upsample_nearest_exact2d_symint(
              grad, output_size, scales_h, scales_w))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  return grad_inputs;
}
void UpsampleNearestExact2DBackwardBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(output_size);
  args.collect(scales_h);
  args.collect(scales_w);
}
variable_list UpsampleNearestExact2DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(output_size);
  saved.before(scales_h);
...
variable_list UpsampleNearest3DBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (upsample_nearest3d_symint(
              grad, output_size, scales_d, scales_h, scales_w))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  return grad_inputs;
...
variable_list UpsampleNearest3DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(output_size);
  saved.before(scales_d);
...
variable_list UpsampleNearestExact3DBackwardBackward0::apply(
    variable_list&& grads) {
  IndexRangeGenerator gen;
  auto grad_output_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result = any_grad_defined
        ? (_upsample_nearest_exact3d_symint(
              grad, output_size, scales_d, scales_h, scales_w))
        : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
...
void UpsampleNearestExact3DBackwardBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(output_size);
  args.collect(scales_d);
  args.collect(scales_h);
...
variable_list UpsampleNearestExact3DBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(output_size);
  saved.before(scales_d);
...
variable_list SigmoidBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result =
        any_grad_defined ? (sigmoid_backward(grad, output.conj())) : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (task_should_compute_output({output_ix})) {
    auto grad_result = any_grad_defined
        ? (grad.conj() * grad_output * (-2 * output.conj() + 1))
        : Tensor();
    copy_range(grad_inputs, output_ix, grad_result);
  }
  return grad_inputs;
...
variable_list SigmoidBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(grad_output_);
  saved.before(output_);
...
variable_list TanhBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({grad_output_ix})) {
    auto grad_result =
        any_grad_defined ? (tanh_backward(grad, output.conj())) : Tensor();
    copy_range(grad_inputs, grad_output_ix, grad_result);
  }
  if (task_should_compute_output({output_ix})) {
    auto grad_result = any_grad_defined
        ? (grad.conj() * (-2 * output.conj() * grad_output))
        : Tensor();
    copy_range(grad_inputs, output_ix, grad_result);
  }
  return grad_inputs;
...
variable_list TanhBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(grad_output_);
  saved.before(output_);
...
variable_list CudnnCtcLossBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({log_probs_ix})) {
    auto grad_result = any_grad_defined
        ? (_cudnn_ctc_loss_backward(grad, result0, result1, zero_infinity))
        : Tensor();
    copy_range(grad_inputs, log_probs_ix, grad_result);
  }
  return grad_inputs;
...
variable_list CudnnCtcLossBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(zero_infinity);
  saved.before(result0_);
...
variable_list CudnnCtcLossBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(zero_infinity);
  saved.before(result0_);
...
variable_list CudnnConvolutionTransposeBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix, weight_ix})) {
    ...
    auto grad_result = _cudnn_convolution_backward(
        self,
        grad,
        weight,
        padding,
...
variable_list CudnnConvolutionTransposeBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dilation);
  saved.before(groups);
...
variable_list MpsConvolutionTransposeBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix, weight_ix})) {
    ...
    auto grad_result = grad.defined()
        ? mps_convolution_transpose_backward_symint(
              self,
              grad,
              weight,
              padding,
...
variable_list MpsConvolutionTransposeBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dilation);
  saved.before(groups);
...
variable_list CudnnConvolutionBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dilation);
  saved.before(groups);
...
variable_list CudnnGridSamplerBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix, grid_ix})) {
    auto grad_result = grad.defined()
        ? cudnn_grid_sampler_backward(self, grid, grad)
        : std::tuple<Tensor, Tensor>();
    if (task_should_compute_output({self_ix})) {
      copy_range(grad_inputs, self_ix, std::get<0>(grad_result));
    }
...
variable_list CudnnGridSamplerBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(grid_);
  saved.before(self_);
...
variable_list CudnnAffineGridGeneratorBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({theta_ix})) {
    auto grad_result = any_grad_defined
        ? (cudnn_affine_grid_generator_backward(grad, N, C, H, W))
        : Tensor();
    copy_range(grad_inputs, theta_ix, grad_result);
  }
  return grad_inputs;
...
variable_list CudnnAffineGridGeneratorBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(C);
  saved.before(H);
...
variable_list CudnnBatchNormBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({input_ix, weight_ix, bias_ix})) {
    ...
    auto grad_result = grad.defined()
        ? (training ? cudnn_batch_norm_backward(
                          input,
                          grad.contiguous(input.suggest_memory_format()),
                          weight,
                          running_mean,
                          running_var,
                          result1,
                          result2,
                          epsilon,
                          retain_variables ? result3.clone() : result3)
...
variable_list CudnnBatchNormBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list CudnnBatchNormBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({reserveSpace_ix})) {
    auto grad_result =
        not_implemented("cudnn_batch_norm_backward reserveSpace");
...
variable_list CudnnBatchNormBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(epsilon);
  saved.before(grad_output_);
  ...
  if (task_should_compute_output({input_ix, weight_ix, bias_ix})) {
    ...
    auto grad_result = grad.defined()
        ? convolution_backward_symint(
              ...
              stride,
              padding,
              std::vector<c10::SymInt>(padding.size(), 1),
              false,
              std::vector<c10::SymInt>(padding.size(), 0),
              1,
              grad_input_mask)
...
variable_list NnpackSpatialConvolutionBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(bias_sym_sizes_opt);
  saved.before(input_);
...
variable_list LstmMpsBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({input_ix, hx_ix, params_ix})) {
    auto grad_result = lstm_mps_backward(
        grads[0],
        grads[1],
        grads[2],
        result3,
...
variable_list LstmMpsBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list CudnnRnnBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({input_ix, hx_ix, cx_ix, weight_ix})) {
    ...
    auto grad_result = _cudnn_rnn_backward_symint(
        input,
        weight,
        weight_stride0,
        result4,
...
variable_list CudnnRnnBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(batch_first);
...
variable_list CudnnRnnBackwardBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({cx_ix})) {
    auto grad_result =
        not_implemented("_cudnn_rnn_backward", kCudnnDoubleBackwardMsg);
  ...
  if (task_should_compute_output({weight_ix})) {
    auto grad_result =
        not_implemented_list("_cudnn_rnn_backward", kCudnnDoubleBackwardMsg);
    copy_range(grad_inputs, weight_ix, grad_result);
  }
  return grad_inputs;
}
void CudnnRnnBackwardBackward0::compiled_args(CompiledNodeArgs& args) {}
variable_list CudnnRnnBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list MiopenConvolutionTransposeBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void MiopenConvolutionTransposeBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(bias_sym_sizes_opt);
  args.collect(dilation);
  args.collect(groups);
...
variable_list MiopenConvolutionTransposeBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(bias_sym_sizes_opt);
  saved.before(dilation);
...
variable_list MiopenConvolutionBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(bias_sym_sizes_opt);
  saved.before(dilation);
...
variable_list MiopenDepthwiseConvolutionBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void MiopenDepthwiseConvolutionBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(bias_sym_sizes_opt);
  args.collect(dilation);
  args.collect(groups);
...
variable_list MiopenDepthwiseConvolutionBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(bias_sym_sizes_opt);
  saved.before(dilation);
...
variable_list MiopenBatchNormBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({input_ix, weight_ix, bias_ix})) {
    ...
    auto grad_result = grad.defined() ? (training ? miopen_batch_norm_backward(
                                                        input,
                                                        grad.contiguous(),
                                                        weight,
                                                        running_mean,
...
variable_list MiopenBatchNormBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(epsilon);
  saved.before(input_);
...
variable_list MiopenBatchNormBackwardBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(epsilon);
  saved.before(grad_output_);
...
variable_list MiopenRnnBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({input_ix, hx_ix, cx_ix, weight_ix})) {
    ...
    auto grad_result = miopen_rnn_backward(
        input,
        weight,
        weight_stride0,
        result4,
...
variable_list MiopenRnnBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(batch_first);
  saved.before(batch_sizes);
...
variable_list MkldnnRnnLayerBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output(
          {input_ix,
           ...
           hx__ix,
           cx__ix})) {
    auto grad_result = GradMode::is_enabled()
...
variable_list MkldnnRnnLayerBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(batch_first);
  saved.before(batch_sizes);
  ...
  if (task_should_compute_output({self_ix, weight_ix, bias_ix})) {
    ...
    auto grad_result = grad.defined()
        ? convolution_backward_symint(
              ...
              dilation,
              /*transposed=*/false,
              /*output_padding=*/std::vector<c10::SymInt>(padding.size(), 0),
              groups,
              grad_input_mask)
        : std::tuple<Tensor, Tensor, Tensor>();
    if (task_should_compute_output({self_ix})) {
...
variable_list MkldnnConvolutionBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(bias_sym_sizes_opt);
  saved.before(dilation);
...
variable_list MkldnnLinearBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix, weight_ix, bias_ix})) {
    ...
    auto grad_result =
        mkldnn_linear_backward(self, grad, weight, grad_input_mask);
    if (task_should_compute_output({self_ix})) {
      copy_range(grad_inputs, self_ix, std::get<0>(grad_result));
    }
    if (task_should_compute_output({weight_ix})) {
...
variable_list MkldnnLinearBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(weight_);
...
variable_list MkldnnMaxPool2DBackward0::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (mkldnn_max_pool2d_backward(
                                              grad,
                                              result,
                                              self,
                                              kernel_size,
...
variable_list MkldnnMaxPool2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(ceil_mode);
  saved.before(dilation);
...
variable_list MkldnnMaxPool3DBackward0::apply(variable_list&& grads) {
  ...
  bool any_grad_defined = any_variable_defined(grads);
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined ? (mkldnn_max_pool3d_backward(
                                              grad,
                                              result,
                                              self,
                                              kernel_size,
...
variable_list MkldnnMaxPool3DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(ceil_mode);
  saved.before(dilation);
...
variable_list MkldnnAdaptiveAvgPool2DBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (mkldnn_adaptive_avg_pool2d_backward(grad, self))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list MkldnnAdaptiveAvgPool2DBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list MkldnnReshapeBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_sym_sizes);
  variable_list result = apply(variable_list(grads));
...
variable_list NestedTensorFromTensorListBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (task_should_compute_output({list_ix})) {
    auto grad_result =
        grad.defined() ? at::unbind(grad) : std::vector<Tensor>(list.size());
    copy_range(grad_inputs, list_ix, grad_result);
  }
  return grad_inputs;
}
void NestedTensorFromTensorListBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(list_);
}
variable_list NestedTensorFromTensorListBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(list_);
  variable_list result = apply(variable_list(grads));
...
variable_list NestedTensorFromMaskBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({t_ix})) {
    auto grad_result = any_grad_defined
        ? (grad.to_padded_tensor_symint(0, t_sym_sizes))
        : Tensor();
...
variable_list NestedTensorFromMaskBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list NestedFromPaddedBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({padded_ix})) {
    auto grad_result = any_grad_defined
        ? (_nested_from_padded_backward(grad, padded, fuse_transform_0213))
        : Tensor();
...
variable_list NestedFromPaddedBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list ToPaddedTensorBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list NestedViewFromBufferBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list ScaledDotProductEfficientAttentionBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (task_should_compute_output({query_ix, key_ix, value_ix, attn_bias_ix})) {
    ...
    auto grad_result = _scaled_dot_product_efficient_attention_backward(
        grad,
        query,
        key,
        value,
...
void ScaledDotProductEfficientAttentionBackward0::compiled_args(
    CompiledNodeArgs& args) {
...
variable_list ScaledDotProductEfficientAttentionBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list ScaledDotProductFlashAttentionBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (task_should_compute_output({query_ix, key_ix, value_ix})) {
    auto grad_result = _scaled_dot_product_flash_attention_backward_symint(
        grad,
        query,
        key,
        value,
...
void ScaledDotProductFlashAttentionBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(dropout_p);
  args.collect(is_causal);
  args.collect(key_);
...
variable_list ScaledDotProductFlashAttentionBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dropout_p);
  saved.before(is_causal);
...
variable_list FlashAttentionBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({query_ix, key_ix, value_ix})) {
    auto grad_result = _flash_attention_backward_symint(
        grad,
        query,
        key,
        value,
...
variable_list FlashAttentionBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(cum_seq_k_);
  saved.before(cum_seq_q_);
...
variable_list EfficientAttentionBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({query_ix, key_ix, value_ix, bias_ix})) {
    auto grad_result = _efficient_attention_backward_symint(
        grad,
        query,
        key,
        value,
...
variable_list EfficientAttentionBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(bias_);
...
variable_list FftR2CBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (fft_r2c_backward(
              grad, dim, normalization, onesided, self.sym_size(dim.back())))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list FftR2CBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
...
variable_list FftC2RBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (fft_c2r_backward(grad, dim, normalization))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list FftC2RBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(normalization);
...
variable_list FftC2CBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result = any_grad_defined
        ? (_fft_c2c_symint(grad, dim, normalization, !forward))
        : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
...
variable_list FftC2CBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
...
variable_list UnbindBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (unbind_backward(grads, dim)) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list UnbindBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  variable_list result = apply(variable_list(grads));
...
variable_list UnbindBackwardAutogradNestedTensor0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void UnbindBackwardAutogradNestedTensor0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(dim);
  args.collect(self_);
  args.collect(self_options);
}
variable_list UnbindBackwardAutogradNestedTensor0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_);
...
variable_list StackBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({tensors_ix})) {
    auto grad_result =
        stack_tensors_backward(grad, dim, tensors_args_scalartypes);
    copy_range(grad_inputs, tensors_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list StackBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(tensors_args_scalartypes);
  ...
  if (task_should_compute_output(
    ...
    auto grad_result = GradMode::is_enabled()
        ...
        : _thnn_fused_lstm_cell_backward(
              grads[0], grads[1], cx, result1, result2, input_bias.defined());
...
variable_list ThnnFusedLstmCellBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(cx_);
...
variable_list ThnnFusedGruCellBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output(
          {input_gates_ix,
           ...
           hidden_bias_ix})) {
    auto grad_result = grad.defined()
        ? (GradMode::is_enabled() ? _thnn_differentiable_gru_cell_backward(
                                        grad,
                                        input_gates,
                                        hidden_gates,
                                        hx,
                                  ...
                                  : _thnn_fused_gru_cell_backward(
                                        grad, result1, input_bias.defined()))
...
variable_list ThnnFusedGruCellBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(hidden_bias_);
  saved.before(hidden_gates_);
...
variable_list PackPaddedSequenceBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({input_ix})) {
    auto grad_result = any_grad_defined
        ? (_pack_padded_sequence_backward_symint(
              grad, input_sym_sizes, result1, batch_first))
        : Tensor();
    copy_range(grad_inputs, input_ix, grad_result);
  }
  return grad_inputs;
...
variable_list PackPaddedSequenceBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(batch_first);
  ...
  if (task_should_compute_output({data_ix})) {
    auto grad_result = any_grad_defined
        ? (_segment_reduce_backward(
              grad, result, data, reduce, lengths, offsets, axis, initial))
        : Tensor();
...
variable_list SegmentReduceBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(axis);
  saved.before(data_);
...
variable_list PinMemoryBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list TestWarnInAutogradBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list TestAutogradMultipleDispatchBackward0::apply(
    variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad.expand_symint(self_sym_sizes) + 1) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
void TestAutogradMultipleDispatchBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(self_sym_sizes);
}
variable_list TestAutogradMultipleDispatchBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_sym_sizes);
  variable_list result = apply(variable_list(grads));
...
variable_list TestAutogradMultipleDispatchBackwardAutogradNestedTensor0::apply(
    variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
...
void TestAutogradMultipleDispatchBackwardAutogradNestedTensor0::compiled_args(
    CompiledNodeArgs& args) {}
variable_list TestAutogradMultipleDispatchBackwardAutogradNestedTensor0::
    apply_with_saved(const variable_list& grads, SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));

  return result;
}
variable_list TestAutogradMultipleDispatchBackwardAutogradCUDA0::apply(
    variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad.expand_symint(self_sym_sizes) * 2) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
void TestAutogradMultipleDispatchBackwardAutogradCUDA0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(self_sym_sizes);
}
variable_list TestAutogradMultipleDispatchBackwardAutogradCUDA0::
    apply_with_saved(const variable_list& grads, SwapSavedVariables& saved) {
  saved.before(self_sym_sizes);
  variable_list result = apply(variable_list(grads));
  saved.after(self_sym_sizes);
...
variable_list TestAutogradMultipleDispatchBackwardAutogradNestedTensor1::apply(
    variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
...
void TestAutogradMultipleDispatchBackwardAutogradNestedTensor1::compiled_args(
    CompiledNodeArgs& args) {}
variable_list TestAutogradMultipleDispatchBackwardAutogradNestedTensor1::
    apply_with_saved(const variable_list& grads, SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));

  return result;
}
variable_list TestAutogradMultipleDispatchViewBackward0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void TestAutogradMultipleDispatchViewBackward0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(self_);
}
variable_list TestAutogradMultipleDispatchViewBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list TestAutogradMultipleDispatchViewBackwardAutogradCUDA0::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (task_should_compute_output({self_ix})) {
    auto grad_result =
        any_grad_defined ? (grad.reshape_as(self) + 1) : Tensor();
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
void TestAutogradMultipleDispatchViewBackwardAutogradCUDA0::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(self_);
}
variable_list TestAutogradMultipleDispatchViewBackwardAutogradCUDA0::
    apply_with_saved(const variable_list& grads, SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
  saved.after(self_);
...
variable_list ScatterReduceBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix, src_ix})) {
    auto grad_result = scatter_reduce_backward(
        grad, self, dim, index, src, reduce, include_self, result);
    if (task_should_compute_output({self_ix})) {
      copy_range(grad_inputs, self_ix, std::get<0>(grad_result));
    }
    if (task_should_compute_output({src_ix})) {
...
variable_list ScatterReduceBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(include_self);
...
variable_list ReshapeCopyBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_sym_sizes);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachDivBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            div_tensor_other_backward(grads[i], self[i], other[i]));
      } else {
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(div_tensor_self_backward(
            grads[i], other[i], self[i].scalar_type()));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachDivBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list ForeachPowBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({exponent_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            pow_backward_exponent(grads[i], self[i], exponent[i], result[i]));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            pow_backward_self(grads[i], self[i], exponent[i]));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachPowBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(exponent_);
  saved.before(self_);
...
variable_list ForeachPowBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(exponent);
  saved.before(self_);
...
variable_list ForeachPowBackward2::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({exponent_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            pow_backward_exponent(grads[i], self, exponent[i], result[i]));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachPowBackward2::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(exponent_);
  saved.before(self);
...
variable_list ForeachMinimumBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            at::where(self[i] == scalar, grads[i] / 2, grads[i])
                .masked_fill_(self[i] > scalar, 0));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachMinimumBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(scalar);
  saved.before(self_);
...
variable_list ForeachMinimumBackward1::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            at::where(self[i] == scalars[i], grads[i] / 2, grads[i])
                .masked_fill_(self[i] > scalars[i], 0));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachMinimumBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
...
variable_list ForeachMaximumBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(scalar);
  saved.before(self_);
...
variable_list ForeachMaximumBackward1::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(scalars);
  saved.before(self_);
...
variable_list ForeachNormBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            norm_backward(grads[i], self[i], ord, result[i]));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachNormBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(ord);
  saved.before(self_);
...
variable_list AliasBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list AsStridedBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_geometry);
  saved.before(size);
...
variable_list ConjBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list NegViewBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list DiagonalBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim1);
  saved.before(dim2);
...
variable_list ExpandBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_sym_sizes);
  variable_list result = apply(variable_list(grads));
...
variable_list PermuteBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dims);
  variable_list result = apply(variable_list(grads));
...
variable_list ReshapeAliasBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_sym_sizes);
  variable_list result = apply(variable_list(grads));
...
variable_list SelectBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(index);
...
variable_list SelectBackwardAutogradNestedTensor0_copy::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void SelectBackwardAutogradNestedTensor0_copy::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(dim);
  args.collect(index);
  args.collect(self_);
}
variable_list SelectBackwardAutogradNestedTensor0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(index);
...
variable_list SliceBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(end);
...
variable_list SplitBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_options);
...
variable_list SplitWithSizesBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_options);
...
variable_list SplitWithSizesBackwardAutogradNestedTensor0_copy::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void SplitWithSizesBackwardAutogradNestedTensor0_copy::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(dim);
  args.collect(self_);
  args.collect(self_options);
...
variable_list SplitWithSizesBackwardAutogradNestedTensor0_copy::
    apply_with_saved(const variable_list& grads, SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_);
  saved.before(self_options);
...
variable_list SqueezeBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_sym_sizes);
  variable_list result = apply(variable_list(grads));
...
variable_list SqueezeBackward1_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_sym_sizes);
...
variable_list SqueezeBackwardAutogradNestedTensor0_copy::apply(
    variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
...
void SqueezeBackwardAutogradNestedTensor0_copy::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(dim);
}
variable_list SqueezeBackwardAutogradNestedTensor0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  variable_list result = apply(variable_list(grads));
...
variable_list SqueezeBackward2_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_sym_sizes);
...
variable_list SqueezeBackwardAutogradNestedTensor1_copy::apply(
    variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
...
void SqueezeBackwardAutogradNestedTensor1_copy::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(dim);
  args.collect(self_dim);
}
variable_list SqueezeBackwardAutogradNestedTensor1_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_dim);
...
variable_list TBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list TransposeBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim0);
  saved.before(dim1);
...
variable_list UnfoldBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dimension);
  saved.before(self_sym_sizes);
...
variable_list LiftFreshBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list UnsqueezeBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  variable_list result = apply(variable_list(grads));
...
variable_list ViewBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_sym_sizes);
  variable_list result = apply(variable_list(grads));
...
variable_list ViewBackwardAutogradNestedTensor0_copy::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void ViewBackwardAutogradNestedTensor0_copy::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(self_);
}
variable_list ViewBackwardAutogradNestedTensor0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ViewAsRealBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list ViewAsComplexBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list ValuesBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(self_sym_sizes);
...
variable_list ValuesBackwardAutogradNestedTensor0_copy::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void ValuesBackwardAutogradNestedTensor0_copy::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(self_);
}
variable_list ValuesBackwardAutogradNestedTensor0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list NestedViewFromBufferBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list UnbindBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  variable_list result = apply(variable_list(grads));
...
variable_list UnbindBackwardAutogradNestedTensor0_copy::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void UnbindBackwardAutogradNestedTensor0_copy::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(dim);
  args.collect(self_);
  args.collect(self_options);
}
variable_list UnbindBackwardAutogradNestedTensor0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(dim);
  saved.before(self_);
...
variable_list TestAutogradMultipleDispatchViewBackward0_copy::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void TestAutogradMultipleDispatchViewBackward0_copy::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(self_);
}
variable_list TestAutogradMultipleDispatchViewBackward0_copy::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list TestAutogradMultipleDispatchViewBackwardAutogradCUDA0_copy::apply(
    variable_list&& grads) {
  std::lock_guard<std::mutex> lock(mutex_);
...
void TestAutogradMultipleDispatchViewBackwardAutogradCUDA0_copy::compiled_args(
    CompiledNodeArgs& args) {
  args.collect(self_);
}
variable_list TestAutogradMultipleDispatchViewBackwardAutogradCUDA0_copy::
    apply_with_saved(const variable_list& grads, SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
  saved.after(self_);
...
variable_list ForeachAbsBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachAcosBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            grads[i] * -((-self[i] * self[i] + 1).rsqrt()).conj());
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachAcosBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachAddBackward1Scalar::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            handle_r_to_c(self[i].scalar_type(), grads[i]));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachAddBackward1Scalar::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachAddBackward0List::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(handle_r_to_c(
            other[i].scalar_type(), maybe_multiply(grads[i], alpha.conj())));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachAddBackward0List::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
  saved.before(other_);
...
variable_list ForeachAddBackward1ScalarList::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachAddBackward0Tensor::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            any_grad_defined ? (handle_r_to_c(
                                   other.scalar_type(),
                                   maybe_multiply(grads[i], alpha.conj())))
                             : Tensor());
      } else {
        grad_result.emplace_back(Tensor());
      }
...
variable_list ForeachAddBackward0Tensor::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
  saved.before(other_);
...
variable_list ForeachAddcdivBackward0Scalar::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({tensor1_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(handle_r_to_c(
            tensor1[i].scalar_type(), grads[i] * (value / tensor2[i]).conj()));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
  ...
  if (task_should_compute_output({tensor2_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(handle_r_to_c(
            tensor2[i][i].scalar_type(),
            -grads[i] *
                (value * tensor1[i] / (tensor2[i] * tensor2[i])).conj()));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachAddcdivBackward0Scalar::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(tensor1_);
...
variable_list ForeachAddcdivBackward0ScalarList::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({tensor1_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(handle_r_to_c(
            tensor1[i].scalar_type(),
            grads[i] * (scalars[i] / tensor2[i]).conj()));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(handle_r_to_c(
            tensor2[i][i].scalar_type(),
            -grads[i] *
                (scalars[i] * tensor1[i] / (tensor2[i] * tensor2[i])).conj()));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachAddcdivBackward0ScalarList::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(scalars);
  saved.before(self_);
...
variable_list ForeachAddcmulBackward0Scalar::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({tensor1_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(handle_r_to_c(
            tensor1[i].scalar_type(), grads[i] * (tensor2[i] * value).conj()));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
  ...
  if (task_should_compute_output({tensor2_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(handle_r_to_c(
            tensor2[i].scalar_type(), grads[i] * (tensor1[i] * value).conj()));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachAddcmulBackward0Scalar::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  saved.before(tensor1_);
...
variable_list ForeachAddcmulBackward0ScalarList::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({tensor1_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(handle_r_to_c(
            tensor1[i].scalar_type(),
            grads[i] * (tensor2[i] * scalars[i]).conj()));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
  ...
  if (task_should_compute_output({tensor2_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(handle_r_to_c(
            tensor2[i].scalar_type(),
            grads[i] * (tensor1[i] * scalars[i]).conj()));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachAddcmulBackward0ScalarList::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(scalars);
  saved.before(self_);
...
variable_list ForeachAsinBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachAtanBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachCeilBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachClampMaxBackward0Scalar::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(where(
            self[i] <= scalar,
            grads[i],
            at::scalar_tensor(0., grads[i].options())));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachClampMaxBackward0Scalar::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(scalar);
  saved.before(self_);
...
variable_list ForeachClampMaxBackward1List::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list ForeachClampMaxBackward0ScalarList::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(scalars);
  saved.before(self_);
...
variable_list ForeachClampMinBackward0Scalar::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(scalar);
  saved.before(self_);
...
variable_list ForeachClampMinBackward1List::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list ForeachClampMinBackward0ScalarList::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(scalars);
  saved.before(self_);
...
variable_list ForeachCosBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachCoshBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachDivBackward1Scalar::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            div_tensor_self_backward(grads[i], scalar, self[i].scalar_type()));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachDivBackward1Scalar::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(scalar);
  saved.before(self_);
...
variable_list ForeachDivBackward1ScalarList::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(div_tensor_self_backward(
            grads[i], scalars[i], self[i].scalar_type()));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachDivBackward1ScalarList::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(scalars);
  saved.before(self_);
...
variable_list ForeachDivBackward0Tensor::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            any_grad_defined
                ? (div_tensor_other_backward(grads[i], self[i], other))
                : Tensor());
      } else {
        grad_result.emplace_back(Tensor());
      }
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            div_tensor_self_backward(grads[i], other, self[i].scalar_type()));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachDivBackward0Tensor::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list ForeachErfBackward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            2.0 / sqrt(M_PI) * exp(-(self[i].pow(2))) * grads[i]);
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachErfBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachErfcBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachExpBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachExpm1Backward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachFloorBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachFracBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachLerpBackward1List::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            weight.isComplex()
                ? grads[i] * (1 - weight.conj().toComplexDouble())
                : grads[i] * (1 - weight.toDouble()));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachLerpBackward0Scalar::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(weight);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachLgammaBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachLogBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachLog10Backward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            grads[i] / (self[i].conj() * 2.3025850929940456));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachLog10Backward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachLog1PBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachLog2Backward0::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            grads[i] / (self[i].conj() * 0.6931471805599453));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachLog2Backward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachMaximumBackward0List::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            at::where(self[i] == other[i], grads[i] / 2, grads[i])
                .masked_fill_(self[i] > other[i], 0));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachMaximumBackward0List::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list ForeachMinimumBackward0List::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list ForeachMulBackward1Scalar::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            mul_tensor_backward(grads[i], scalar, self[i].scalar_type()));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachMulBackward1Scalar::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(scalar);
  saved.before(self_);
...
variable_list ForeachMulBackward0List::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            mul_tensor_backward(grads[i], self[i], other[i].scalar_type()));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            mul_tensor_backward(grads[i], other[i], self[i].scalar_type()));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachMulBackward0List::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list ForeachMulBackward1ScalarList::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            mul_tensor_backward(grads[i], scalars[i], self[i].scalar_type()));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachMulBackward1ScalarList::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(scalars);
  saved.before(self_);
...
variable_list ForeachMulBackward0Tensor::apply(variable_list&& grads) {
  ...
  if (task_should_compute_output({other_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            any_grad_defined
                ? (mul_tensor_backward(grads[i], self[i], other.scalar_type()))
                : Tensor());
      } else {
        grad_result.emplace_back(Tensor());
      }
  ...
  if (task_should_compute_output({self_ix})) {
    ...
    for (const auto& i : c10::irange(grads.size())) {
      if (grads[i].defined()) {
        grad_result.emplace_back(
            mul_tensor_backward(grads[i], other, self[i].scalar_type()));
      } else {
        grad_result.emplace_back(Tensor());
      }
    }
...
variable_list ForeachMulBackward0Tensor::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(other_);
  saved.before(self_);
...
variable_list ForeachNegBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachPowBackward0Scalar::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(exponent);
  saved.before(self_);
...
variable_list ForeachReciprocalBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachRoundBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachSigmoidBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachSignBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachSinBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachSinhBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachSqrtBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachSubBackward1Scalar::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachSubBackward0List::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(alpha);
  saved.before(other_);
...
variable_list ForeachSubBackward1ScalarList::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(self_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachTanBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachTanhBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  saved.before(result_);
  variable_list result = apply(variable_list(grads));
...
variable_list ForeachTruncBackward0::apply_with_saved(
    const variable_list& grads,
    SwapSavedVariables& saved) {
  variable_list result = apply(variable_list(grads));
